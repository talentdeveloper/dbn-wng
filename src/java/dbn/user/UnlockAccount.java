...
import static dbn.user.JSONResponses.LOCK_ACCOUNT;
...
public final class UnlockAccount extends UserServlet.UserRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IOException {
        ...
        if (account == null) {
            ...
            try (DbIterator<? extends Transaction> transactions = Dbn.getTransactionProcessor().getAllUnconfirmedTransactions()) {
                while (transactions.hasNext()) {
                    ...
                    if (Arrays.equals(transaction.getSenderPublicKey(), accountPublicKey)) {
                        ...
                        myTransaction.put("sentAmountDQT", transaction.getAmountDQT());
                        if (accountId == transaction.getRecipientId()) {
                            myTransaction.put("receivedAmountDQT", transaction.getAmountDQT());
                        }
                        myTransaction.put("feeDQT", transaction.getFeeDQT());
            ...
            int blockchainHeight = Dbn.getBlockchain().getLastBlock().getHeight();
            try (DbIterator<? extends Block> blockIterator = Dbn.getBlockchain().getBlocks(accountId, 0)) {
                while (blockIterator.hasNext()) {
                    Block block = blockIterator.next();
                    if (block.getTotalFeeDQT() > 0) {
                        JSONObject myTransaction = new JSONObject();
                        ...
                        myTransaction.put("earnedAmountDQT", block.getTotalFeeDQT());
            ...
            try (DbIterator<? extends Transaction> transactionIterator = Dbn.getBlockchain().getTransactions(accountId, (byte) -1, (byte) -1, 0, false)) {
