    ...
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IOException {
        ...
        try (DbIterator<? extends Transaction> transactions = Dbn.getTransactionProcessor().getAllUnconfirmedTransactions()) {
            while (transactions.hasNext()) {
                ...
                unconfirmedTransaction.put("amountDQT", transaction.getAmountDQT());
                unconfirmedTransaction.put("feeDQT", transaction.getFeeDQT());
        ...
        try (DbIterator<? extends Block> lastBlocks = Dbn.getBlockchain().getBlocks(0, 59)) {
            for (Block block : lastBlocks) {
                ...
                recentBlock.put("totalAmountDQT", block.getTotalAmountDQT());
                recentBlock.put("totalFeeDQT", block.getTotalFeeDQT());
