 ...
 * <p>
 * DBN, ASSET and CURRENCY balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = Dbn.getIntProperty("dbn.maxNumberOfMonitors");
     ...
     * @param   holdingId           Asset or Currency identifier, ignored for DBN monitor
    ...
    private FundingMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        this.holdingId = (holdingType != HoldingType.DBN ? holdingId : 0);
        ...
        synchronized(monitors) {
            ...
            while (monitorIt.hasNext()) {
                ...
                if (monitor.holdingType == holdingType && monitor.property.equals(property) &&
                        (holdingType == HoldingType.DBN || monitor.holdingId == holdingId) &&
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            Dbn.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (Dbn.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case DBN:
                                        processDbnEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
     ...
     * @throws  DbnException                Unable to create transaction
     */
    private static void processDbnEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
                                            throws DbnException {
        FundingMonitor monitor = monitoredAccount.monitor;
        if (targetAccount.getBalanceDQT() < monitoredAccount.threshold) {
            Transaction.Builder builder = Dbn.newTransactionBuilder(monitor.publicKey,
                    monitoredAccount.amount, 0, (short)1440, Attachment.ORDINARY_PAYMENT);
            builder.recipientId(monitoredAccount.accountId)
                   .timestamp(Dbn.getBlockchain().getLastBlockTimestamp());
            Transaction transaction = builder.build(monitor.secretPhrase);
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeDQT()) > fundingAccount.getUnconfirmedBalanceDQT()) {
                ...
                monitoredAccount.height = Dbn.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("DBN funding transaction %s for %f DBN submitted from %s to %s",
                        transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_DBN,
    ...
    private static void processAssetEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
        ...
        if (fundingAsset == null || fundingAsset.getUnconfirmedQuantityQNT() < monitoredAccount.amount) {
            ...
            builder.recipientId(monitoredAccount.accountId)
                   .timestamp(Dbn.getBlockchain().getLastBlockTimestamp());
            Transaction transaction = builder.build(monitor.secretPhrase);
            if (transaction.getFeeDQT() > fundingAccount.getUnconfirmedBalanceDQT()) {
                Logger.logWarningMessage(String.format("Funding account %s has insufficient funds; funding transaction discarded",
                        monitor.accountName));
            } else {
                Dbn.getTransactionProcessor().broadcast(transaction);
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            long balance = account.getBalanceDQT();
            //
            // Check the DBN balance for monitored accounts
            //
            synchronized(monitors) {
                List<MonitoredAccount> accountList = accounts.get(account.getId());
                if (accountList != null) {
                    accountList.forEach((maccount) -> {
                       if (maccount.monitor.holdingType == HoldingType.DBN && balance < maccount.threshold &&
