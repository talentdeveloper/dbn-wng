...
import static dbn.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static long getAmountDQT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "amountDQT", 1L, Constants.MAX_BALANCE_DQT, true);
    }

    public static long getFeeDQT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "feeDQT", 0L, Constants.MAX_BALANCE_DQT, true);
    }

    public static long getPriceDQT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "priceDQT", 1L, Constants.MAX_BALANCE_DQT, true);
    }
    ...
    public static long getAmountDQTPerQNT(HttpServletRequest req) throws ParameterException {
        return getLong(req, "amountDQTPerQNT", 1L, Constants.MAX_BALANCE_DQT, true);
    }
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, Dbn.getBlockchain().getHeight(), false);
    }
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.DBN);
        if (holdingType == HoldingType.DBN && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is DBN"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return Dbn.newTransactionBuilder(json);
            } catch (DbnException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return Dbn.newTransactionBuilder(bytes, prunableAttachments);
            } catch (DbnException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, DbnException.NotValidException {
