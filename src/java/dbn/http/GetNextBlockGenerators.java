...
public final class GetNextBlockGenerators extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (block == null) {
            curBlock = Dbn.getBlockchain().getLastBlock();
        } else {
            try {
                curBlock = Dbn.getBlockchain().getBlock(Convert.parseUnsignedLong(block));
                if (curBlock == null) {
        ...
        */
        ...
        Block curBlock = Dbn.getBlockchain().getLastBlock();
        ...
        while (iterator.hasNext() && hubs.size() < limit) {
            ...
            hub.put("minFeePerByteDQT", hit.hub.getMinFeePerByteDQT());
