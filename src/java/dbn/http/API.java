...
import static dbn.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static dbn.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static dbn.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = Dbn.getStringProperty("dbn.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Dbn.getIntProperty("dbn.maxAPIRecords");
    static final boolean enableAPIUPnP = Dbn.getBooleanProperty("dbn.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Dbn.getIntProperty("dbn.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Dbn.getBooleanProperty("dbn.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(Dbn.getStringListProperty("dbn.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Dbn.getStringListProperty("dbn.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Dbn.getStringListProperty("dbn.allowedBotHosts");
        ...
        boolean enableAPIServer = Dbn.getBooleanProperty("dbn.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Dbn.getIntProperty("dbn.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Dbn.getIntProperty("dbn.apiServerSSLPort");
            final String host = Dbn.getStringProperty("dbn.apiServerHost");
            disableAdminPassword = Dbn.getBooleanProperty("dbn.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Dbn.getBooleanProperty("dbn.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Dbn.getUserHomeDir()).resolve(Paths.get(Dbn.getStringProperty("dbn.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Dbn.getStringProperty("dbn.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Dbn.getStringProperty("dbn.keyStoreType"));
                List<String> ciphers = Dbn.getStringListProperty("dbn.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Dbn.getStringProperty("dbn.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Dbn.getStringProperty("dbn.apiWelcomeFile")});
            }

            String javadocResourceBase = Dbn.getStringProperty("dbn.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/dbn");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Dbn.getIntProperty("dbn.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/dbn-proxy");
            ...
            if (!Dbn.getBooleanProperty("dbn.enableAPIServerGZIPFilter")) {
                gzipHandler.setExcludedPaths("/dbn", "/dbn-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(dbn.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Dbn.getBooleanProperty("dbn.apiFrameOptionsSameOrigin")) {
