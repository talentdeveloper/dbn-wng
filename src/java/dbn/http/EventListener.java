...
import dbn.TransactionProcessor;
 ...
 * has not been received within dbn.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by dbn.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Dbn.getIntProperty("dbn.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Dbn.getIntProperty("dbn.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Dbn.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Dbn.getTransactionProcessor();
    ...
    /** Dbn event listeners */
    private final List<DbnEventListener> dbnEventListeners = new ArrayList<>();
     ...
     *
     * Dbn event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Dbn event registrations
     ...
     *
     * @param   eventRegistrations      Dbn event registrations
     * @throws  EventListenerException  Invalid Dbn event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<DbnEventListener> it = dbnEventListeners.iterator();
                while (it.hasNext()) {
                    DbnEventListener listener = it.next();
                ...
                if (addListener) {
                    DbnEventListener listener = new DbnEventListener(event);
                    listener.addListener();
                    dbnEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            dbnEventListeners.forEach(DbnEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Dbn event listener
     */
    private class DbnEventListener {

        /** Event handler */
        private final DbnEventHandler eventHandler;

        /**
         * Create the Dbn event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public DbnEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Dbn listener for this event
         */
        ...
        /**
         * Remove the Dbn listener for this event
         */
        ...
        /**
         * Return the hash code for this Dbn event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Dbn events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof DbnEventListener) &&
                    eventHandler.equals(((DbnEventListener)obj).eventHandler));
        }

        /**
         * Dbn listener event handler
         */
        private abstract class DbnEventHandler {
            ...
            /**
             * Create the Dbn event handler
             *
             * @param   eventRegistration   Event registration
             */
            public DbnEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof DbnEventHandler) &&
                                        owner == ((DbnEventHandler)obj).owner &&
                                        accountId == ((DbnEventHandler)obj).accountId &&
                                        event == ((DbnEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends DbnEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends DbnEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends DbnEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends DbnEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Dbn listener event
        ...
        /**
         * Return the Dbn listener event
         *
         * @return                  Dbn listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
