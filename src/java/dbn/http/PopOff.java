...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Dbn.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Dbn.getBlockchainProcessor().popOffTo(Dbn.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Dbn.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Dbn.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Dbn.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
