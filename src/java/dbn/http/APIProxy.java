...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Dbn.getBooleanProperty("dbn.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = Dbn.getIntProperty("dbn.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Dbn.getStringProperty("dbn.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Dbn.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, Dbn.getEpochTime() + blacklistingPeriod);
