...
import static dbn.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static dbn.http.JSONResponses.INCORRECT_DEADLINE;
import static dbn.http.JSONResponses.INCORRECT_EC_BLOCK;
import static dbn.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static dbn.http.JSONResponses.INCORRECT_WHITELIST;
import static dbn.http.JSONResponses.MISSING_DEADLINE;
import static dbn.http.JSONResponses.MISSING_SECRET_PHRASE;
import static dbn.http.JSONResponses.NOT_ENOUGH_FUNDS;

abstract class CreateTransaction extends APIServlet.APIRequestHandler {

    private static final String[] commonParameters = new String[]{"secretPhrase", "publicKey", "feeDQT",
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId, long amountDQT)
            throws DbnException {
        return createTransaction(req, senderAccount, recipientId, amountDQT, Attachment.ORDINARY_PAYMENT);
    }
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Dbn.getBlockchain().getHeight() + 1,
                Dbn.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountDQT, Attachment attachment) throws DbnException {
        ...
        long feeDQT = ParameterParser.getFeeDQT(req);
        ...
        if (ecBlockId != 0 && ecBlockId != Dbn.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Dbn.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Dbn.newTransactionBuilder(publicKey, amountDQT, feeDQT,
            ...
            try {
                if (Math.addExact(amountDQT, transaction.getFeeDQT()) > senderAccount.getUnconfirmedBalanceDQT()) {
            ...
            } catch (DbnException.NotYetEncryptedException ignore) {}
        ...
        } catch (DbnException.NotYetEnabledException e) {
