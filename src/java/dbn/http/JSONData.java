...
import dbn.AccountRestrictions;
import dbn.Alias;
import dbn.Appendix;
import dbn.Asset;
import dbn.AssetDelete;
import dbn.AssetDividend;
import dbn.AssetTransfer;
import dbn.Attachment;
import dbn.Block;
import dbn.Constants;
import dbn.Currency;
import dbn.CurrencyExchangeOffer;
import dbn.CurrencyFounder;
import dbn.CurrencyTransfer;
import dbn.CurrencyType;
import dbn.DigitalGoodsStore;
import dbn.Exchange;
import dbn.ExchangeRequest;
import dbn.FundingMonitor;
import dbn.Generator;
import dbn.HoldingType;
import dbn.MonetarySystem;
import dbn.Dbn;
import dbn.Order;
import dbn.PhasingPoll;
import dbn.PhasingVote;
import dbn.Poll;
import dbn.PrunableMessage;
import dbn.Shuffler;
import dbn.Shuffling;
import dbn.ShufflingParticipant;
...
public final class JSONData {
    ...
    static JSONObject alias(Alias alias) {
        ...
        if (offer != null) {
            json.put("priceDQT", String.valueOf(offer.getPriceDQT()));
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Dbn.getBlockchain().getHeight());
    }

    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        JSONObject json = new JSONObject();
        if (account == null) {
            json.put("balanceDQT", "0");
            json.put("unconfirmedBalanceDQT", "0");
            json.put("forgedBalanceDQT", "0");
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDBN", "0");
                json.put("guaranteedBalanceDQT", "0");
            }
        } else {
            json.put("balanceDQT", String.valueOf(account.getBalanceDQT()));
            json.put("unconfirmedBalanceDQT", String.valueOf(account.getUnconfirmedBalanceDQT()));
            json.put("forgedBalanceDQT", String.valueOf(account.getForgedBalanceDQT()));
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDBN", account.getEffectiveBalanceDBN(height));
                json.put("guaranteedBalanceDQT", String.valueOf(account.getGuaranteedBalanceDQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, height)));
            }
        }
        return json;
    }
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceDBN", String.valueOf(account.getGuaranteedBalanceDQT() / Constants.ONE_DBN));
            }
        }
    ...
    static JSONObject currency(Currency currency, boolean includeCounts) {
        ...
        json.put("minReservePerUnitDQT", String.valueOf(currency.getMinReservePerUnitDQT()));
        json.put("currentReservePerUnitDQT", String.valueOf(currency.getCurrentReservePerUnitDQT()));
    ...
    static JSONObject currencyFounder(CurrencyFounder founder) {
        ...
        json.put("amountPerUnitDQT", String.valueOf(founder.getAmountPerUnitDQT()));
        return json;
    }
    ...
    private static JSONObject order(Order order) {
        ...
        json.put("priceDQT", String.valueOf(order.getPriceDQT()));
    ...
    private static JSONObject expectedOrder(Transaction transaction) {
        ...
        json.put("priceDQT", String.valueOf(attachment.getPriceDQT()));
    ...
    static JSONObject offer(CurrencyExchangeOffer offer) {
        ...
        json.put("rateDQT", String.valueOf(offer.getRateDQT()));
    ...
    static JSONObject expectedBuyOffer(Transaction transaction) {
        ...
        json.put("rateDQT", String.valueOf(attachment.getBuyRateDQT()));
    ...
    static JSONObject expectedSellOffer(Transaction transaction) {
        JSONObject json = expectedOffer(transaction);
        Attachment.MonetarySystemPublishExchangeOffer attachment = (Attachment.MonetarySystemPublishExchangeOffer)transaction.getAttachment();
        json.put("rateDQT", String.valueOf(attachment.getSellRateDQT()));
    ...
    static JSONObject availableOffers(CurrencyExchangeOffer.AvailableOffers availableOffers) {
        JSONObject json = new JSONObject();
        json.put("rateDQT", String.valueOf(availableOffers.getRateDQT()));
        json.put("units", String.valueOf(availableOffers.getUnits()));
        json.put("amountDQT", String.valueOf(availableOffers.getAmountDQT()));
        return json;
    }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.DBN) {
    ...
    static JSONObject block(Block block, boolean includeTransactions, boolean includeExecutedPhased) {
        ...
        json.put("totalAmountDQT", String.valueOf(block.getTotalAmountDQT()));
        json.put("totalFeeDQT", String.valueOf(block.getTotalFeeDQT()));
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Dbn.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject goods(DigitalGoodsStore.Goods goods, boolean includeCounts) {
        ...
        json.put("priceDQT", String.valueOf(goods.getPriceDQT()));
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Dbn.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject purchase(DigitalGoodsStore.Purchase purchase) {
        ...
        json.put("priceDQT", String.valueOf(purchase.getPriceDQT()));
        ...
        if (purchase.getDiscountDQT() > 0) {
            json.put("discountDQT", String.valueOf(purchase.getDiscountDQT()));
        }
        if (purchase.getRefundDQT() > 0) {
            json.put("refundDQT", String.valueOf(purchase.getRefundDQT()));
        }
        return json;
    }
    ...
    static JSONObject trade(Trade trade, boolean includeAssetInfo) {
        ...
        json.put("priceDQT", String.valueOf(trade.getPriceDQT()));
    ...
    static JSONObject assetDividend(AssetDividend assetDividend) {
        ...
        json.put("amountDQTPerQNT", String.valueOf(assetDividend.getAmountDQTPerQNT()));
    ...
    static JSONObject exchange(Exchange exchange, boolean includeCurrencyInfo) {
        ...
        json.put("rateDQT", String.valueOf(exchange.getRate()));
    ...
    static JSONObject exchangeRequest(ExchangeRequest exchangeRequest, boolean includeCurrencyInfo) {
        ...
        json.put("rateDQT", String.valueOf(exchangeRequest.getRate()));
    ...
    static JSONObject expectedExchangeRequest(Transaction transaction, boolean includeCurrencyInfo) {
        ...
        json.put("units", String.valueOf(attachment.getUnits()));
        json.put("rateDQT", String.valueOf(attachment.getRateDQT()));
        if (includeCurrencyInfo) {
            putCurrencyInfo(json, attachment.getCurrencyId());
        }
        putExpectedTransaction(json, transaction);
    ...
    static JSONObject unconfirmedTransaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("amountDQT", String.valueOf(transaction.getAmountDQT()));
        json.put("feeDQT", String.valueOf(transaction.getFeeDQT()));
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Dbn.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Dbn.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Dbn.getBlockchain().getTransaction(entry.getEventId());
