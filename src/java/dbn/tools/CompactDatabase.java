 ...
 *
 *   java -cp "classes:lib/*:conf" dbn.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Ddbn.runtime.mode=desktop dbn.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Dbn properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "dbn.testDb" : "dbn.db";
        String dbType = Dbn.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Dbn.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Dbn.getDbDir(Dbn.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Dbn.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Dbn.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Dbn.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "dbn.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "dbn.mv.db");
            if (!dbFile.exists()) {
