...
/**
 * Handle logging for the Dbn node server
 */
public final class Logger {
     ...
     * The dbn/conf/logging-default.properties and dbn/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "dbn.util.DbnLogManager");
        if (!(LogManager.getLogManager() instanceof DbnLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("dbn.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Dbn.loadProperties(loggingProperties, "logging-default.properties", true);
                Dbn.loadProperties(loggingProperties, "logging.properties", false);
                Dbn.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(dbn.Dbn.class);
        enableStackTraces = Dbn.getBooleanProperty("dbn.enableStackTraces");
        enableLogTraceback = Dbn.getBooleanProperty("dbn.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof DbnLogManager) {
            ((DbnLogManager) LogManager.getLogManager()).dbnShutdown();
        }
    }
