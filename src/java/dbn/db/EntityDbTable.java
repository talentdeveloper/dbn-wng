...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Dbn.getBlockchainProcessor().isScanning() ?
                    Math.max(Dbn.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return Dbn.getBlockchain().getHeight() <= height && ! (isPersistent() && Dbn.getBlockchainProcessor().isScanning());
    }
