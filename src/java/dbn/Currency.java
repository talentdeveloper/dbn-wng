...
public final class Currency {
    ...
    private static final class CurrencySupply {
        ...
        private long currentReservePerUnitDQT;
        ...
        private CurrencySupply(ResultSet rs, DbKey dbKey) throws SQLException {
            ...
            this.currentReservePerUnitDQT = rs.getLong("current_reserve_per_unit_nqt");
        }
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO currency_supply (id, current_supply, "
                ...
                pstmt.setLong(++i, this.currentReservePerUnitDQT);
    ...
    static {
        Dbn.getBlockchainProcessor().addListener(new CrowdFundingListener(), BlockchainProcessor.Event.AFTER_BLOCK_APPLY);
    }
    ...
    private Currency(Transaction transaction, Attachment.MonetarySystemCurrencyIssuance attachment) {
        ...
        this.minReservePerUnitDQT = attachment.getMinReservePerUnitDQT();
    ...
    private Currency(ResultSet rs, DbKey dbKey) throws SQLException {
        ...
        this.minReservePerUnitDQT = rs.getLong("min_reserve_per_unit_nqt");
    ...
    private void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO currency (id, account_id, name, code, "
            ...
            pstmt.setLong(++i, this.minReservePerUnitDQT);
    ...
    public long getCurrentReservePerUnitDQT() {
        if (!is(CurrencyType.RESERVABLE) || getSupplyData() == null) {
            return 0;
        }
        return currencySupply.currentReservePerUnitDQT;
    }
    ...
    static void increaseReserve(LedgerEvent event, long eventId, Account account, long currencyId, long amountPerUnitDQT) {
        Currency currency = Currency.getCurrency(currencyId);
        account.addToBalanceDQT(event, eventId, -Math.multiplyExact(currency.getReserveSupply(), amountPerUnitDQT));
        CurrencySupply currencySupply = currency.getSupplyData();
        currencySupply.currentReservePerUnitDQT += amountPerUnitDQT;
        currencySupplyTable.insert(currencySupply);
        CurrencyFounder.addOrUpdateFounder(currencyId, account.getId(), amountPerUnitDQT);
    }
    ...
    static void claimReserve(LedgerEvent event, long eventId, Account account, long currencyId, long units) {
        ...
        account.addToBalanceAndUnconfirmedBalanceDQT(event, eventId,
                Math.multiplyExact(units, currency.getCurrentReservePerUnitDQT()));
    }
                ...
                try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currencyId, 0, Integer.MAX_VALUE)) {
                    for (CurrencyFounder founder : founders) {
                        Account.getAccount(founder.getAccountId())
                                .addToBalanceAndUnconfirmedBalanceDQT(event, eventId, Math.multiplyExact(reserveSupply,
                                        founder.getAmountPerUnitDQT()));
                    }
                }
            }
    ...
    private static final class CrowdFundingListener implements Listener<Block> {
        ...
        public void notify(Block block) {
            ...
            try (DbIterator<Currency> issuedCurrencies = currencyTable.getManyBy(new DbClause.IntClause("issuance_height", block.getHeight()), 0, -1)) {
                for (Currency currency : issuedCurrencies) {
                    if (currency.getCurrentReservePerUnitDQT() < currency.getMinReservePerUnitDQT()) {
        ...
        private void undoCrowdFunding(Currency currency) {
            try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
                for (CurrencyFounder founder : founders) {
                    Account.getAccount(founder.getAccountId())
                            .addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent.CURRENCY_UNDO_CROWDFUNDING, currency.getId(),
        ...
        private void distributeCurrency(Currency currency) {
            ...
            if (!currency.is(CurrencyType.CLAIMABLE)) {
                issuerAccount.addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent.CURRENCY_DISTRIBUTION, currency.getId(),
