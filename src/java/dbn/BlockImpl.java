...
final class BlockImpl implements Block {
    ...
    private final long totalAmountDQT;
    private final long totalFeeDQT;
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountDQT, long totalFeeDQT, int payloadLength, byte[] payloadHash,
        ...
        this(version, timestamp, previousBlockId, totalAmountDQT, totalFeeDQT, payloadLength, payloadHash,
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountDQT, long totalFeeDQT, int payloadLength, byte[] payloadHash,
        ...
        this.totalAmountDQT = totalAmountDQT;
        this.totalFeeDQT = totalFeeDQT;
    ...
    BlockImpl(int version, int timestamp, long previousBlockId, long totalAmountDQT, long totalFeeDQT, int payloadLength,
    ...
    @Override
    public long getTotalAmountDQT() {
        return totalAmountDQT;
    }

    @Override
    public long getTotalFeeDQT() {
        return totalFeeDQT;
    }
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("totalAmountDQT", totalAmountDQT);
        json.put("totalFeeDQT", totalFeeDQT);
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws DbnException.NotValidException {
        try {
            int version = ((Long) blockData.get("version")).intValue();
            int timestamp = ((Long) blockData.get("timestamp")).intValue();
            long previousBlock = Convert.parseUnsignedLong((String) blockData.get("previousBlock"));
            long totalAmountDQT = Convert.parseLong(blockData.get("totalAmountDQT"));
            long totalFeeDQT = Convert.parseLong(blockData.get("totalFeeDQT"));
            ...
            BlockImpl block = new BlockImpl(version, timestamp, previousBlock, totalAmountDQT, totalFeeDQT, payloadLength, payloadHash, generatorPublicKey,
                    generationSignature, blockSignature, previousBlockHash, blockTransactions);
            if (!block.checkSignature()) {
                throw new DbnException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountDQT / Constants.ONE_DBN));
                buffer.putInt((int) (totalFeeDQT / Constants.ONE_DBN));
            } else {
                buffer.putLong(totalAmountDQT);
                buffer.putLong(totalFeeDQT);
            }
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceDBN();
    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f DBN to forger at height %d", ((double)backFees[i])/Constants.ONE_DBN, this.height - i - 1);
                previousGeneratorAccount.addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent.BLOCK_GENERATED, getId(), backFees[i]);
                previousGeneratorAccount.addToForgedBalanceDQT(backFees[i]);
            }
        }
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f DBN at height %d", ((double)totalBackFees)/Constants.ONE_DBN, this.height);
        }
        generatorAccount.addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent.BLOCK_GENERATED, getId(), totalFeeDQT - totalBackFees);
        generatorAccount.addToForgedBalanceDQT(totalFeeDQT - totalBackFees);
    }
