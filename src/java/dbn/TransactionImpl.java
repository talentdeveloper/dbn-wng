...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        BuilderImpl(byte version, byte[] senderPublicKey, long amountDQT, long feeDQT, short deadline,
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws DbnException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = Dbn.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws DbnException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws DbnException.NotValidException {

        this.timestamp = builder.timestamp;
        this.deadline = builder.deadline;
        this.senderPublicKey = builder.senderPublicKey;
        this.recipientId = builder.recipientId;
        this.amountDQT = builder.amountDQT;
        ...
        if (builder.feeDQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Dbn.getBlockchain().getHeight());
            long minFee = getMinimumFeeDQT(effectiveHeight);
            feeDQT = Math.max(minFee, builder.feeDQT);
        } else {
            feeDQT = builder.feeDQT;
        }

        if (builder.signature != null && secretPhrase != null) {
            throw new DbnException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new DbnException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
    ...
    byte[] bytes() {
        if (bytes == null) {
            try {
                ...
                if (useDQT()) {
                    buffer.putLong(amountDQT);
                    buffer.putLong(feeDQT);
                    ...
                    buffer.putInt((int) (amountDQT / Constants.ONE_DBN));
                    buffer.putInt((int) (feeDQT / Constants.ONE_DBN));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws DbnException.NotValidException {
        try {
            ...
            long amountDQT = buffer.getLong();
            long feeDQT = buffer.getLong();
            ...
            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey, amountDQT, feeDQT,
            ...
            if (buffer.hasRemaining()) {
                throw new DbnException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (DbnException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws DbnException.NotValidException {
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("amountDQT", amountDQT);
        json.put("feeDQT", feeDQT);
        if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws DbnException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new DbnException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws DbnException.NotValidException {
        try {
            ...
            long amountDQT = Convert.parseLong(transactionData.get("amountDQT"));
            long feeDQT = Convert.parseLong(transactionData.get("feeDQT"));
            ...
            if (transactionType == null) {
                throw new DbnException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey,
                    amountDQT, feeDQT, deadline,
    ...
    private boolean checkSignature() {
        if (!hasValidSignature) {
            hasValidSignature = signature != null && Crypto.verify(signature, zeroSignature(getBytes()), getSenderPublicKey(), useDQT());
        }
    ...
    private int signatureOffset() {
        return 1 + 1 + 4 + 2 + 32 + 8 + (useDQT() ? 8 + 8 + 32 : 4 + 4 + 8);
    }

    private boolean useDQT() {
        return this.height > Constants.DQT_BLOCK
                && (this.timestamp > (Constants.isTestnet ? 12908200 : 14271000)
                || Dbn.getBlockchain().getHeight() >= Constants.DQT_BLOCK);
    }
    ...
    @Override
    public void validate() throws DbnException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeDQT != 0) : (deadline < 1 || feeDQT <= 0)
                || feeDQT > Constants.MAX_BALANCE_DQT
                || amountDQT < 0
                || amountDQT > Constants.MAX_BALANCE_DQT
                || type == null) {
            throw new DbnException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                    + ", deadline: " + deadline + ", fee: " + feeDQT + ", amount: " + amountDQT);
        }

        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new DbnException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new DbnException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountDQT() != 0) {
                throw new DbnException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new DbnException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion(this.version)) {
                throw new DbnException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new DbnException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }

        if (!validatingAtFinish) {
            int blockchainHeight = Dbn.getBlockchain().getHeight();
            long minimumFeeDQT = getMinimumFeeDQT(blockchainHeight);
            if (feeDQT < minimumFeeDQT) {
                throw new DbnException.NotCurrentlyValidException(String.format("Transaction fee %f DBN less than minimum fee %f DBN at height %d",
                        ((double) feeDQT) / Constants.ONE_DBN, ((double) minimumFeeDQT) / Constants.ONE_DBN, blockchainHeight));
            }
            if (blockchainHeight > Constants.FXT_BLOCK && ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new DbnException.NotCurrentlyValidException("ecBlockHeight " + ecBlockHeight
    ...
    void apply() {
        ...
        if (referencedTransactionFullHash != null
            ...
            senderAccount.addToUnconfirmedBalanceDQT(getType().getLedgerEvent(), getId(),
                    0, Constants.UNCONFIRMED_POOL_DEPOSIT_DQT);
        }
        if (attachmentIsPhased()) {
            senderAccount.addToBalanceDQT(getType().getLedgerEvent(), getId(), 0, -feeDQT);
        }
        for (Appendix.AbstractAppendix appendage : appendages) {
    ...
    private long getMinimumFeeDQT(int blockchainHeight) {
        long totalFee = 0;
        for (Appendix.AbstractAppendix appendage : appendages) {
            appendage.loadPrunable(this);
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_DBN);
        }
        return totalFee;
    }
