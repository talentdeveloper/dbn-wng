...
public final class Shuffler {
    ...
    private static final int MAX_SHUFFLERS = Dbn.getIntProperty("dbn.maxNumberOfShufflers");
    ...
    private static void scheduleExpiration(Shuffling shuffling) {
        int expirationHeight = Dbn.getBlockchain().getHeight() + 720;
    ...
    private volatile DbnException.NotCurrentlyValidException failureCause;
    ...
    public DbnException.NotCurrentlyValidException getFailureCause() {
        return failureCause;
    }
    ...
    private void submitTransaction(Attachment.ShufflingAttachment attachment) {
        ...
        try {
            Transaction.Builder builder = Dbn.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, 0,
                    (short) 1440, attachment);
            builder.timestamp(Dbn.getBlockchain().getLastBlockTimestamp());
            Transaction transaction = builder.build(secretPhrase);
            failedTransaction = null;
            failureCause = null;
            Account participantAccount = Account.getAccount(this.accountId);
            if (participantAccount == null || transaction.getFeeDQT() > participantAccount.getUnconfirmedBalanceDQT()) {
                failedTransaction = transaction;
                failureCause = new DbnException.NotCurrentlyValidException("Insufficient balance");
    ...
    public static class ShufflerException extends DbnException {
