...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Dbn.getBooleanProperty("dbn.deleteFinishedShufflings");
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (holdingType != HoldingType.DBN) {
                    participantAccount.addToBalanceDQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_DQT);
                }
            }
        }
        for (byte[] recipientPublicKey : recipientPublicKeys) {
            ...
            if (holdingType != HoldingType.DBN) {
                recipientAccount.addToBalanceAndUnconfirmedBalanceDQT(event, this.id, Constants.SHUFFLING_DEPOSIT_DQT);
            }
        }
    ...
    private void cancel(Block block) {
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (participantAccount.getId() != blamedAccountId) {
                    if (holdingType != HoldingType.DBN) {
                        participantAccount.addToUnconfirmedBalanceDQT(event, this.id, Constants.SHUFFLING_DEPOSIT_DQT);
                    }
                } else {
                    if (holdingType == HoldingType.DBN) {
                        participantAccount.addToUnconfirmedBalanceDQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_DQT);
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                previousGeneratorAccount.addToBalanceAndUnconfirmedBalanceDQT(AccountLedger.LedgerEvent.BLOCK_GENERATED, block.getId(), fee);
                previousGeneratorAccount.addToForgedBalanceDQT(fee);
                Logger.logDebugMessage("Shuffling penalty %f DBN awarded to forger at height %d", ((double)fee) / Constants.ONE_DBN, block.getHeight() - i - 1);
            }
            fee = Constants.SHUFFLING_DEPOSIT_DQT - 3 * fee;
            Account blockGeneratorAccount = Account.getAccount(block.getGeneratorId());
            blockGeneratorAccount.addToBalanceAndUnconfirmedBalanceDQT(AccountLedger.LedgerEvent.BLOCK_GENERATED, block.getId(), fee);
            blockGeneratorAccount.addToForgedBalanceDQT(fee);
            Logger.logDebugMessage("Shuffling penalty %f DBN awarded to forger at height %d", ((double)fee) / Constants.ONE_DBN, block.getHeight());
        }
