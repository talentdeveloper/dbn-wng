...
final class TransactionDb {
    ...
    static TransactionImpl loadTransaction(Connection con, ResultSet rs) throws DbnException.NotValidException {
        try {
            ...
            long amountDQT = rs.getLong("amount");
            long feeDQT = rs.getLong("fee");
            ...
            TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl(version, null,
                    amountDQT, feeDQT, deadline, transactionType.parseAttachment(buffer, version))
                    .timestamp(timestamp)
        ...
        try {
            ...
            for (TransactionImpl transaction : transactions) {
                try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO transaction (id, deadline, "
                    ...
                    pstmt.setLong(++i, transaction.getAmountDQT());
                    pstmt.setLong(++i, transaction.getFeeDQT());
