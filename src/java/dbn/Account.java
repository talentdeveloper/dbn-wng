...
import dbn.AccountLedger.LedgerEntry;
import dbn.AccountLedger.LedgerEvent;
import dbn.AccountLedger.LedgerHolding;
...
import dbn.db.DerivedDbTable;
import dbn.db.VersionedEntityDbTable;
import dbn.db.VersionedPersistentDbTable;
import dbn.util.Convert;
import dbn.util.Listener;
import dbn.util.Listeners;
...
public final class Account {
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Dbn.getBlockchain().getHeight();
        }
        ...
        private void save(Connection con) throws SQLException {
            height = Dbn.getBlockchain().getHeight();
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Dbn.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }
            if (height > Dbn.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Dbn.getBlockchain().getHeight());
            }
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Dbn.getBooleanProperty("dbn.enablePublicKeyCache") ?
    ...
    static {
        ...
        if (publicKeyCache != null) {
            ...
            Dbn.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    private long balanceDQT;
    private long unconfirmedBalanceDQT;
    private long forgedBalanceDQT;
    ...
    private Account(ResultSet rs, DbKey dbKey) throws SQLException {
        this.id = rs.getLong("id");
        this.dbKey = dbKey;
        this.balanceDQT = rs.getLong("balance");
        this.unconfirmedBalanceDQT = rs.getLong("unconfirmed_balance");
        this.forgedBalanceDQT = rs.getLong("forged_balance");
    ...
    private void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account (id, "
            ...
            int i = 0;
            pstmt.setLong(++i, this.id);
            pstmt.setLong(++i, this.balanceDQT);
            pstmt.setLong(++i, this.unconfirmedBalanceDQT);
            pstmt.setLong(++i, this.forgedBalanceDQT);
    ...
    private void save() {
        if (balanceDQT == 0 && unconfirmedBalanceDQT == 0 && forgedBalanceDQT == 0 && activeLesseeId == 0 && controls.isEmpty()) {
    ...
    public long getBalanceDQT() {
        return balanceDQT;
    }

    public long getUnconfirmedBalanceDQT() {
        return unconfirmedBalanceDQT;
    }

    public long getForgedBalanceDQT() {
        return forgedBalanceDQT;
    }

    public long getEffectiveBalanceDBN() {
        return getEffectiveBalanceDBN(Dbn.getBlockchain().getHeight());
    }

    public long getEffectiveBalanceDBN(int height) {
        if (height >= Constants.TRANSPARENT_FORGING_BLOCK_6) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            ...
            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceDQT / Constants.ONE_DBN;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : Dbn.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountDQT();
                }
            }
            return (balanceDQT - receivedInLastBlock) / Constants.ONE_DBN;
        }
        Dbn.getBlockchain().readLock();
        try {
            long effectiveBalanceDQT = getLessorsGuaranteedBalanceDQT(height);
            if (activeLesseeId == 0) {
                effectiveBalanceDQT += getGuaranteedBalanceDQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, height);
            }
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceDQT < Constants.MIN_FORGING_BALANCE_DQT) ? 0 : effectiveBalanceDQT / Constants.ONE_DBN;
        } finally {
            Dbn.getBlockchain().readUnlock();
        }
    }

    private long getLessorsGuaranteedBalanceDQT(int height) {
        ...
        for (int i = 0; i < lessors.size(); i++) {
            ...
            balances[i] = lessors.get(i).getBalanceDQT();
        }
        int blockchainHeight = Dbn.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
    ...
    public long getGuaranteedBalanceDQT() {
        return getGuaranteedBalanceDQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Dbn.getBlockchain().getHeight());
    }

    public long getGuaranteedBalanceDQT(final int numberOfConfirmations, final int currentHeight) {
        Dbn.getBlockchain().readLock();
        try {
            int height = currentHeight - numberOfConfirmations;
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Dbn.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Dbn.getBlockchain().getHeight()) {
            ...
            try (Connection con = Db.db.getConnection();
                ...
                try (ResultSet rs = pstmt.executeQuery()) {
                    ...
                    return Math.max(Math.subtractExact(balanceDQT, rs.getLong("additions")), 0);
                }
            } catch (SQLException e) {
                throw new RuntimeException(e.toString(), e);
            }
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Dbn.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Dbn.getBlockchain().getHeight() - 1) {
    ...
    void addToBalanceDQT(LedgerEvent event, long eventId, long amountDQT) {
        addToBalanceDQT(event, eventId, amountDQT, 0);
    }

    void addToBalanceDQT(LedgerEvent event, long eventId, long amountDQT, long feeDQT) {
        if (amountDQT == 0 && feeDQT == 0) {
            return;
        }
        long totalAmountDQT = Math.addExact(amountDQT, feeDQT);
        this.balanceDQT = Math.addExact(this.balanceDQT, totalAmountDQT);
        addToGuaranteedBalanceDQT(totalAmountDQT);
        checkBalance(this.id, this.balanceDQT, this.unconfirmedBalanceDQT);
        save();
        listeners.notify(this, Event.BALANCE);
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeDQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.DBN_BALANCE, null, feeDQT, this.balanceDQT - amountDQT));
            }
            if (amountDQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.DBN_BALANCE, null, amountDQT, this.balanceDQT));
            }
        }
    }

    void addToUnconfirmedBalanceDQT(LedgerEvent event, long eventId, long amountDQT) {
        addToUnconfirmedBalanceDQT(event, eventId, amountDQT, 0);
    }

    void addToUnconfirmedBalanceDQT(LedgerEvent event, long eventId, long amountDQT, long feeDQT) {
        if (amountDQT == 0 && feeDQT == 0) {
            return;
        }
        long totalAmountDQT = Math.addExact(amountDQT, feeDQT);
        this.unconfirmedBalanceDQT = Math.addExact(this.unconfirmedBalanceDQT, totalAmountDQT);
        checkBalance(this.id, this.balanceDQT, this.unconfirmedBalanceDQT);
        save();
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeDQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_DBN_BALANCE, null, feeDQT, this.unconfirmedBalanceDQT - amountDQT));
            }
            if (amountDQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_DBN_BALANCE, null, amountDQT, this.unconfirmedBalanceDQT));
            }
        }
    }

    void addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent event, long eventId, long amountDQT) {
        addToBalanceAndUnconfirmedBalanceDQT(event, eventId, amountDQT, 0);
    }

    void addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent event, long eventId, long amountDQT, long feeDQT) {
        if (amountDQT == 0 && feeDQT == 0) {
            return;
        }
        long totalAmountDQT = Math.addExact(amountDQT, feeDQT);
    ...
    void addToForgedBalanceDQT(long amountDQT) {
        if (amountDQT == 0) {
            return;
        }
        this.forgedBalanceDQT = Math.addExact(this.forgedBalanceDQT, amountDQT);
        save();
    }
    ...
    private void addToGuaranteedBalanceDQT(long amountDQT) {
        if (amountDQT <= 0) {
            return;
        }
        int blockchainHeight = Dbn.getBlockchain().getHeight();
    ...
    void payDividends(final long transactionId, Attachment.ColoredCoinsDividendPayment attachment) {
        ...
        final long amountDQTPerQNT = attachment.getAmountDQTPerQNT();
        long numAccounts = 0;
        for (final AccountAsset accountAsset : accountAssets) {
            if (accountAsset.getAccountId() != this.id && accountAsset.getQuantityQNT() != 0) {
                long dividend = Math.multiplyExact(accountAsset.getQuantityQNT(), amountDQTPerQNT);
                Account.getAccount(accountAsset.getAccountId())
                        .addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent.ASSET_DIVIDEND_PAYMENT, transactionId, dividend);
                totalDividend += dividend;
                numAccounts += 1;
            }
        }
        this.addToBalanceDQT(LedgerEvent.ASSET_DIVIDEND_PAYMENT, transactionId, -totalDividend);
