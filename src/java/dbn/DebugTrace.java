...
public final class DebugTrace {
    ...
    static final String QUOTE = Dbn.getStringProperty("dbn.debugTraceQuote", "\"");
    static final String SEPARATOR = Dbn.getStringProperty("dbn.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Dbn.getBooleanProperty("dbn.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Dbn.getStringListProperty("dbn.debugTraceAccounts");
        String logName = Dbn.getStringProperty("dbn.debugTraceLog");
        ...
        Dbn.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Dbn.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Dbn.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Dbn.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void trace(Block block) {
        for (Transaction transaction : block.getTransactions()) {
            ...
            if (transaction.getAmountDQT() > 0 && recipientId == 0) {
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            map.put("transaction fee", String.valueOf(-Constants.SHUFFLING_DEPOSIT_DQT));
            map.put("event", "shuffling blame");
            log(map);
            long fee = Constants.SHUFFLING_DEPOSIT_DQT / 4;
            ...
            fee = Constants.SHUFFLING_DEPOSIT_DQT - 3 * fee;
            long generatorId = Dbn.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("lessor guaranteed balance", String.valueOf(account.getGuaranteedBalanceDQT()));
        map.put("lessee", Long.toUnsignedString(lesseeId));
        map.put("timestamp", String.valueOf(Dbn.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Dbn.getBlockchain().getHeight()));
    ...
    private void crowdfunding(Currency currency) {
        ...
        try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
            for (CurrencyFounder founder : founders) {
                totalAmountPerUnit += founder.getAmountPerUnitDQT();
                currencyFounders.add(founder);
            }
        }
        for (CurrencyFounder founder : currencyFounders) {
            long units = Math.multiplyExact(remainingSupply, founder.getAmountPerUnitDQT()) / totalAmountPerUnit;
        ...
        if (!currency.is(CurrencyType.CLAIMABLE)) {
            map.put("currency cost", String.valueOf(Math.multiplyExact(currency.getReserveSupply(), currency.getCurrentReservePerUnitDQT())));
        }
    ...
    private void undoCrowdfunding(Currency currency) {
        try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
            for (CurrencyFounder founder : founders) {
                Map<String,String> founderMap = getValues(founder.getAccountId(), false);
                founderMap.put("currency", Long.toUnsignedString(currency.getId()));
                founderMap.put("currency cost", String.valueOf(Math.multiplyExact(currency.getReserveSupply(), founder.getAmountPerUnitDQT())));
    ...
    private void delete(Currency currency) {
        ...
        if (currency.is(CurrencyType.RESERVABLE)) {
            if (currency.is(CurrencyType.CLAIMABLE) && currency.isActive()) {
                map.put("currency cost", String.valueOf(Math.multiplyExact(units, currency.getCurrentReservePerUnitDQT())));
            }
            if (!currency.isActive()) {
                try (DbIterator<CurrencyFounder> founders = CurrencyFounder.getCurrencyFounders(currency.getId(), 0, Integer.MAX_VALUE)) {
                    for (CurrencyFounder founder : founders) {
    ...
    private Map<String,String> getValues(long accountId, boolean unconfirmed) {
        ...
        Account account = Account.getAccount(accountId);
        map.put("balance", String.valueOf(account != null ? account.getBalanceDQT() : 0));
        map.put("unconfirmed balance", String.valueOf(account != null ? account.getUnconfirmedBalanceDQT() : 0));
        map.put("timestamp", String.valueOf(Dbn.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Dbn.getBlockchain().getHeight()));
    ...
    private Map<String,String> getValues(long accountId, Trade trade, boolean isAsk) {
        ...
        map.put("trade price", String.valueOf(trade.getPriceDQT()));
        long tradeCost = Math.multiplyExact(trade.getQuantityQNT(), trade.getPriceDQT());
    ...
    private Map<String,String> getValues(long accountId, Shuffling shuffling, boolean isRecipient) {
        ...
        String deposit = String.valueOf(isRecipient ? Constants.SHUFFLING_DEPOSIT_DQT : -Constants.SHUFFLING_DEPOSIT_DQT);
        if (shuffling.getHoldingType() == HoldingType.DBN) {
    ...
    private Map<String,String> getValues(long accountId, Transaction transaction, boolean isRecipient, boolean logFee, boolean logAmount) {
        ...
        long fee = transaction.getFeeDQT();
        if (isRecipient) {
    ...
    private Map<String,String> getValues(long accountId, Block block) {
        ...
        if (block.getHeight() > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                if (include(previousGeneratorId)) {
                    ...
                    map.put("effective balance", String.valueOf(Account.getAccount(previousGeneratorId).getEffectiveBalanceDBN()));
        ...
        Map<String,String> map = getValues(accountId, false);
        map.put("effective balance", String.valueOf(Account.getAccount(accountId).getEffectiveBalanceDBN()));
    ...
    private Map<String,String> getValues(long accountId, Transaction transaction, Attachment attachment, boolean isRecipient) {
        ...
        if (attachment instanceof Attachment.ColoredCoinsOrderPlacement) {
            ...
            map.put("order price", String.valueOf(orderPlacement.getPriceDQT()));
            long quantity = orderPlacement.getQuantityQNT();
            if (isAsk) {
                quantity = - quantity;
            }
            map.put("order quantity", String.valueOf(quantity));
            BigInteger orderCost = BigInteger.valueOf(orderPlacement.getPriceDQT()).multiply(BigInteger.valueOf(orderPlacement.getQuantityQNT()));
            if (! isAsk) {
            ...
            long discount = delivery.getDiscountDQT();
            map.put("purchase price", String.valueOf(purchase.getPriceDQT()));
            map.put("purchase quantity", String.valueOf(purchase.getQuantity()));
            long cost = Math.multiplyExact(purchase.getPriceDQT(), (long) purchase.getQuantity());
            if (isRecipient) {
                cost = - cost;
            }
            ...
            long refundDQT = refund.getRefundDQT();
            if (! isRecipient) {
                refundDQT = - refundDQT;
            }
            map.put("refund", String.valueOf(refundDQT));
            ...
            map.put("buy rate", String.valueOf(publishOffer.getBuyRateDQT()));
            map.put("sell rate", String.valueOf(publishOffer.getSellRateDQT()));
            ...
            BigInteger buyCost = BigInteger.valueOf(publishOffer.getBuyRateDQT()).multiply(BigInteger.valueOf(buyUnits));
            map.put("buy cost", buyCost.toString());
            BigInteger sellCost = BigInteger.valueOf(publishOffer.getSellRateDQT()).multiply(BigInteger.valueOf(sellUnits));
            ...
            map.put("currency cost", String.valueOf(Math.multiplyExact(claim.getUnits(), currency.getCurrentReservePerUnitDQT())));
            ...
            map.put("currency cost", String.valueOf(-Math.multiplyExact(reserveIncrease.getAmountPerUnitDQT(), currency.getReserveSupply())));
            ...
            try (DbIterator<Account.AccountAsset> iterator = Account.getAssetAccounts(dividendPayment.getAssetId(), dividendPayment.getHeight(), 0, -1)) {
                while (iterator.hasNext()) {
                    ...
                    if (accountAsset.getAccountId() != accountId && accountAsset.getQuantityQNT() != 0) {
                        long dividend = Math.multiplyExact(accountAsset.getQuantityQNT(), dividendPayment.getAmountDQTPerQNT());
