...
public final class DigitalGoodsStore {
    ...
    static {
        Dbn.getBlockchainProcessor().addListener(block -> {
            ...
            for (Purchase purchase : expiredPurchases) {
                ...
                buyer.addToUnconfirmedBalanceDQT(LedgerEvent.DIGITAL_GOODS_PURCHASE_EXPIRED, purchase.getId(),
                        Math.multiplyExact((long) purchase.getQuantity(), purchase.getPriceDQT()));
    ...
    public static final class Goods {
        ...
        private Goods(Transaction transaction, Attachment.DigitalGoodsListing attachment) {
            ...
            this.priceDQT = attachment.getPriceDQT();
            this.delisted = false;
            this.timestamp = Dbn.getBlockchain().getLastBlockTimestamp();
        ...
        private Goods(ResultSet rs, DbKey dbKey) throws SQLException {
            ...
            this.priceDQT = rs.getLong("price");
        ...
        private void changePrice(long priceDQT) {
    ...
    public static final class Purchase {
        ...
        private static DbIterator<Purchase> getExpiredPendingPurchases(Block block) {
            ...
            final int previousTimestamp = Dbn.getBlockchain().getBlock(block.getPreviousBlockId()).getTimestamp();
        ...
        private long discountDQT;
        private long refundDQT;
        ...
        private Purchase(ResultSet rs, DbKey dbKey) throws SQLException {
            ...
            this.discountDQT = rs.getLong("discount");
            this.refundDQT = rs.getLong("refund");
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO purchase (id, buyer_id, goods_id, seller_id, "
                ...
                pstmt.setLong(++i, this.discountDQT);
                pstmt.setLong(++i, this.refundDQT);
                pstmt.setInt(++i, Dbn.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
        ...
        public long getDiscountDQT() {
            return discountDQT;
        }

        private void setDiscountDQT(long discountDQT) {
            this.discountDQT = discountDQT;
            purchaseTable.insert(this);
        }

        public long getRefundDQT() {
            return refundDQT;
        }

        private void setRefundDQT(long refundDQT) {
    ...
    static void changePrice(long goodsId, long priceDQT) {
        Goods goods = Goods.goodsTable.get(Goods.goodsDbKeyFactory.newKey(goodsId));
        if (! goods.isDelisted()) {
            goods.changePrice(priceDQT);
    ...
    static void purchase(Transaction transaction,  Attachment.DigitalGoodsPurchase attachment) {
        ...
        if (! goods.isDelisted()
                ...
                && attachment.getPriceDQT() == goods.getPriceDQT()) {
            ...
            buyer.addToUnconfirmedBalanceDQT(LedgerEvent.DIGITAL_GOODS_DELISTED, transaction.getId(),
    ...
    static void deliver(Transaction transaction, Attachment.DigitalGoodsDelivery attachment) {
        ...
        purchase.setPending(false);
        long totalWithoutDiscount = Math.multiplyExact((long) purchase.getQuantity(), purchase.getPriceDQT());
        Account buyer = Account.getAccount(purchase.getBuyerId());
        long transactionId = transaction.getId();
        buyer.addToBalanceDQT(LedgerEvent.DIGITAL_GOODS_DELIVERY, transactionId,
                Math.subtractExact(attachment.getDiscountDQT(), totalWithoutDiscount));
        buyer.addToUnconfirmedBalanceDQT(LedgerEvent.DIGITAL_GOODS_DELIVERY, transactionId, attachment.getDiscountDQT());
        Account seller = Account.getAccount(transaction.getSenderId());
        seller.addToBalanceAndUnconfirmedBalanceDQT(LedgerEvent.DIGITAL_GOODS_DELIVERY, transactionId,
                Math.subtractExact(totalWithoutDiscount, attachment.getDiscountDQT()));
        purchase.setEncryptedGoods(attachment.getGoods(), attachment.goodsIsText());
        purchase.setDiscountDQT(attachment.getDiscountDQT());
        purchaseListeners.notify(purchase, Event.DELIVERY);
    }

    static void refund(LedgerEvent event, long eventId, long sellerId, long purchaseId, long refundDQT,
        ...
        seller.addToBalanceDQT(event, eventId, -refundDQT);
        Account buyer = Account.getAccount(purchase.getBuyerId());
        buyer.addToBalanceAndUnconfirmedBalanceDQT(event, eventId, refundDQT);
        if (encryptedMessage != null) {
            purchase.setRefundNote(encryptedMessage.getEncryptedData());
        }
        purchase.setRefundDQT(refundDQT);
