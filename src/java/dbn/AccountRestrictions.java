...
import dbn.DbnException.AccountControlException;
...
public final class AccountRestrictions {
    ...
    public static final class PhasingOnly {
        ...
        private void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws AccountControlException {
            if (!validatingAtFinish && maxFees > 0 && Math.addExact(transaction.getFeeDQT(), PhasingPoll.getSenderPhasedTransactionFees(transaction.getSenderId())) > maxFees) {
                throw new AccountControlException(String.format("Maximum total fees limit of %f DBN exceeded", ((double)maxFees)/Constants.ONE_DBN));
            }
            ...
            if (!validatingAtFinish) {
                int duration = phasingAppendix.getFinishHeight() - Dbn.getBlockchain().getHeight();
    ...
    static void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws DbnException.NotCurrentlyValidException {
        Account senderAccount = Account.getAccount(transaction.getSenderId());
        if (senderAccount == null) {
            throw new DbnException.NotCurrentlyValidException("Account " + Long.toUnsignedString(transaction.getSenderId()) + " does not exist yet");
        }
