...
jar/exe/dmg package, or run "java -jar dbn-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/dbn/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the Dbn server.


Using the dbn-client.zip package:

Unpack the dbn-client.zip package and open a shell in the resulting dbn
...
the dbn_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.dbn for
configuration files, logs, and the dbn_db database. Unlike run.sh, start.sh
...
message "Dbn server 1.x.x started successfully" in the console log. If run in
desktop mode, a JavaFX window will open automatically. Otherwise, open a
browser, without stopping the java process, and go to http://localhost:7826 ,
where the Dbn UI should now be available.
...
to the Dbn installation directory, as the use of special characters may result
...
conf/dbn-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
dbn-default.properties directly, but create a new conf/dbn.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from dbn-default.properties, the
settings in dbn.properties override those in dbn-default.properties. This way,
when upgrading the software, you can safely overwrite dbn-default.properties
with the updated file from the new package, while your customizations remain
safe in the dbn.properties file.


How to contribute?

There are many ways to contribute to Dbn. Here are some examples:
...
The Dbn software is a client-server application. It consists of a java server
...
The user interface is available on port 7826. This port also accepts http API
requests which other Dbn client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
dbn_db directory. When upgrading, you should not delete the old dbn_db
...
dbn_db, except that it will take some extra time to download the blockchain
from scratch.

The default Dbn client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file dbn.log, which gets
overwritten at restart. In case of an error, the dbn.log file may contain
helpful information, so include its contents when submitting a bug report.

In addition to the default user interface at http://localhost:7826 , the
following urls are available:

http://localhost:7826/test - a list of all available http API requests, very
...
http://localhost:7826/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:7826/doc - a javadoc documentation for client developers who
...
dbn.jar file using the enclosed jar.sh script, and then dbn.jar should be
