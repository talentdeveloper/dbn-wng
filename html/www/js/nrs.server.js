...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var dbnFields = [
                ...
                ["priceDBN", "priceDQT"],
                ["refundDBN", "refundDQT"],
                ["discountDBN", "discountDQT"],
                ["phasingQuorumDBN", "phasingQuorum"],
                ["phasingMinBalanceDBN", "phasingMinBalance"],
                ["controlQuorumDBN", "controlQuorum"],
                ["controlMinBalanceDBN", "controlMinBalance"],
                ["controlMaxFeesDBN", "controlMaxFees"],
                ["minBalanceDBN", "minBalance"],
                ["shufflingAmountDBN", "amount"],
                ["monitorAmountDBN", "amount"],
                ["monitorThresholdDBN", "threshold"]
            ];

            for (i = 0; i < dbnFields.length; i++) {
                var dbnField = dbnFields[i][0];
                var nqtField = dbnFields[i][1];
                if (dbnField in data) {
                    data[nqtField] = NRS.convertToDQT(data[dbnField]);
                    delete data[dbnField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeDQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToDBN(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (dbnAddress.set(accountId)) {
                accountRS = dbnAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/dbn";
        } else {
