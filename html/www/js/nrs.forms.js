...
var NRS = (function(NRS, $) {
    ...
    NRS.submitForm = function($modal, $btn) {
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("DBN", "").replace("DQT", "").capitalize();
				var value = $(this).val();
		...
		if (data.recipient) {
			...
			if (/^\d+$/.test(data.recipient)) {
				...
				NRS.unlockForm($modal, $btn);
				return;
			} else if (!/^DBN\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
				var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
				if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^DBN\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
		...
		if (!NRS.showedFormWarning) {
			if ("amountDBN" in data && NRS.settings["amount_warning"] && NRS.settings["amount_warning"] != "0") {
				try {
					var amountDQT = NRS.convertToDQT(data.amountDBN);
				} catch (err) {
				...
				if (new BigInteger(amountDQT).compareTo(new BigInteger(NRS.settings["amount_warning"])) > 0) {
					NRS.showedFormWarning = true;
					$form.find(".error_message").html($.t("error_max_amount_warning", {
						"dbn": NRS.formatAmount(NRS.settings["amount_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			if ("feeDBN" in data && NRS.settings["fee_warning"] && NRS.settings["fee_warning"] != "0") {
				try {
					var feeDQT = NRS.convertToDQT(data.feeDBN);
				} catch (err) {
				...
				if (new BigInteger(feeDQT).compareTo(new BigInteger(NRS.settings["fee_warning"])) > 0) {
					NRS.showedFormWarning = true;
					$form.find(".error_message").html($.t("error_max_fee_warning", {
						"dbn": NRS.formatAmount(NRS.settings["fee_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			var convertDBNFields = ["phasingQuorumDBN", "phasingMinBalanceDBN"];
			$.each(convertDBNFields, function(key, field) {
				if (field in data) {
					try {
						NRS.convertToDQT(data[field]);
					} catch (err) {
	...
	function formResponse(response, data, requestType, $modal, $form, $btn, successMessage,
		...
		if (response.fullHash) {
			NRS.unlockForm($modal, $btn);
			if (data.calculateFee) {
				updateFee($modal, response.transactionJSON.feeDQT);
				return;
			}
    ...
    function updateFee(modal, feeDQT) {
        var fee = $("#" + modal.attr('id').replace('_modal', '') + "_fee");
        fee.val(NRS.convertToDBN(feeDQT));
        var recalcIndicator = $("#" + modal.attr('id').replace('_modal', '') + "_recalc");
