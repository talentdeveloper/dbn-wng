...
var NRS = (function (NRS, $, undefined) {
    ...
    search.on("submit", function (e, data) {
        ...
        $("#buy_currency_with_dbn").html($.t("buy_currency_param", { currency: currencyCode}));
        $("#buy_currency_offers").html($.t("offers_to_buy_currency_param", { currency: currencyCode}));
        $("#sell_currency_with_dbn").html($.t("sell_currency_param", { currency: currencyCode}));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                if (NRS.accountInfo.unconfirmedBalanceDQT == "0") {
                    $("#ms_your_dbn_balance").html("0");
                } else {
                    $("#ms_your_dbn_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceDQT));
                }
                NRS.pageLoaded();
                callback(null);
            }
    ...
    NRS.getCurrencyRows = function (response) {
        ...
        for (var i = 0; i < response.currencies.length; i++) {
            ...
            var minReserve = NRS.escapeRespStr(currency.minReservePerUnitDQT);
    ...
    function processOffers(offers, type, refresh) {
        ...
        if (offers && offers.length) {
            ...
            var rateDQTDecimals = NRS.getNumberOfDecimals(offers, "rateDQT", function(offer) {
                return NRS.formatOrderPricePerWholeQNT(offer.rateDQT, decimals);
            });
            for (i = 0; i < offers.length; i++) {
                var offer = offers[i];
                var rateDQT = offer.rateDQT;
                if (i == 0 && !refresh) {
                    $("#" + (type == "sell" ? "buy" : "sell") + "_currency_rate").val(NRS.calculateOrderPricePerWholeQNT(rateDQT, decimals));
                }
                rows += "<tr>" +
                    ...
                    "<td class='numeric'>" + NRS.formatOrderPricePerWholeQNT(rateDQT, decimals, rateDQTDecimals) + "</td>" +
                    "</tr>";
            }
    ...
    NRS.loadCurrencyOffers = function (type, currencyId, refresh) {
        ...
        function(err, results) {
            ...
            offers.sort(function (a, b) {
                if (type == "sell") {
                    return a.rateDQT - b.rateDQT;
                } else {
                    return b.rateDQT - a.rateDQT;
                }
            });
    ...
    foundersModal.on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrencyFounders", {
            ...
            var minReservePerUnitDQT = new BigInteger(String($invoker.data("minreserve"))).multiply(new BigInteger("" + Math.pow(10, decimals)));
            ...
            if (response.founders && response.founders.length) {
                var amountPerUnitDQT = BigInteger.ZERO;
                for (var i = 0; i < response.founders.length; i++) {
                    amountPerUnitDQT = new BigInteger(response.founders[i].amountPerUnitDQT).multiply(new BigInteger("" + Math.pow(10, decimals)));
                    totalAmountReserved = totalAmountReserved.add(amountPerUnitDQT);
                }
                for (i = 0; i < response.founders.length; i++) {
                    var account = response.founders[i].accountRS;
                    amountPerUnitDQT = new BigInteger(response.founders[i].amountPerUnitDQT).multiply(new BigInteger("" + Math.pow(10, decimals)));
                    var percentage = NRS.calculatePercentage(amountPerUnitDQT, minReservePerUnitDQT);
                    rows += "<tr>" +
                    "<td>" + NRS.getAccountLink(response.founders[i], "account")+ "</td>" +
                    "<td>" + NRS.convertToDBN(amountPerUnitDQT) + "</td>" +
                    "<td>" + NRS.convertToDBN(amountPerUnitDQT.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
                    "<td>" + NRS.formatQuantity(resSupply.subtract(initialSupply).multiply(amountPerUnitDQT).divide(totalAmountReserved), decimals) + "</td>" +
                    "<td>" + percentage + "</td>" +
                    "</tr>";
                }
            } else {
            ...
            rows += "<tr>" +
            "<td><b>Totals</b></td>" +
            "<td>" + NRS.convertToDBN(totalAmountReserved) + "</td>" +
            "<td>" + NRS.convertToDBN(totalAmountReserved.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
            "<td>" + NRS.formatQuantity(resSupply.subtract(initialSupply), decimals) + "</td>" +
            "<td>" + NRS.calculatePercentage(totalAmountReserved, minReservePerUnitDQT) + "</td>" +
            "</tr>";
        ...
        if (NRS.currenciesTradeHistoryType == "my_exchanges") {
            NRS.sendRequest("getExchanges+", {
                ...
                if (response.exchanges && response.exchanges.length) {
                    ...
                    var rateDQTDecimals = NRS.getNumberOfDecimals(response.exchanges, "rateDQT", function(exchange) {
                        return NRS.formatOrderPricePerWholeQNT(exchange.rateDQT, decimals);
                    });
                    var totalDQTDecimals = NRS.getNumberOfDecimals(response.exchanges, "totalDQT", function(exchange) {
                        return NRS.formatAmount(NRS.calculateOrderTotalDQT(exchange.units, exchange.rateDQT));
                    });
                    for (var i = 0; i < response.exchanges.length; i++) {
                        ...
                        "<td class='numeric'>" + NRS.formatOrderPricePerWholeQNT(exchange.rateDQT, exchange.decimals, rateDQTDecimals) + "</td>" +
                        "<td class='numeric'>" + NRS.formatAmount(NRS.calculateOrderTotalDQT(exchange.units, exchange.rateDQT), false, false, totalDQTDecimals) + "</td>" +
                        "</tr>";
                    }
    ...
    function processExchangeRequests(exchangeRequests, refresh) {
        ...
        if (exchangeRequests && exchangeRequests.length) {
            ...
            var rateDQTDecimals = NRS.getNumberOfDecimals(exchangeRequests, "rateDQT", function(exchangeRequest) {
                return NRS.formatOrderPricePerWholeQNT(exchangeRequest.rateDQT, decimals);
            });
            var totalDQTDecimals = NRS.getNumberOfDecimals(exchangeRequests, "totalDQT", function(exchangeRequest) {
                return NRS.formatAmount(NRS.calculateOrderTotalDQT(exchangeRequest.units, exchangeRequest.rateDQT));
            });
            for (i = 0; i < exchangeRequests.length; i++) {
                ...
                rows += "<tr class=confirmed>" +
                    ...
                    "<td class='numeric'>" + NRS.formatOrderPricePerWholeQNT(exchangeRequest.rateDQT, decimals, rateDQTDecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatAmount(NRS.calculateOrderTotalDQT(exchangeRequest.units, exchangeRequest.rateDQT), false, false, totalDQTDecimals) + "</td>" +
                    "</tr>";
            }
    ...
    $("#currency_order_modal").on("show.bs.modal", function (e) {
        ...
        var rateDQT = $invoker.data("rateDQT");
        var effectiveRate = $invoker.data("effectiveRate");
        var totalDQT = $invoker.data("totalDQT");
        var totalDBN = NRS.formatAmount(totalDQT, false, true);
        ...
        $("#currency_order_total").html(totalDBN + " DBN");
        ...
        $("#currency_order_rate").val(rateDQT);
    });
    ...
    unitFields.on("change", function() {
        ...
        NRS.sendRequest("getAvailableTo" + NRS.initialCaps(orderType), {
            ...
            var rate = NRS.calculateOrderPricePerWholeQNT(response.rateDQT, decimals);
            rateField.val(rate);
            var amount = NRS.convertToDBN(response.amountDQT);
            ...
            submitButton.data("rateDQT", response.rateDQT);
            submitButton.data("effectiveRate", effectiveRate);
            submitButton.data("totalDQT", response.amountDQT);
            submitButton.prop('disabled', false);
        })
   	});
    ...
    NRS.pages.exchange_history = function () {
        NRS.sendRequest("getExchanges+", {
            ...
            if (response.exchanges && response.exchanges.length) {
                ...
                var rateDQTDecimals = NRS.getNumberOfDecimals(response.exchanges, "rateDQT", function(exchange) {
                    return NRS.formatOrderPricePerWholeQNT(exchange.rateDQT, exchange.decimals);
                });
                var totalDQTDecimals = NRS.getNumberOfDecimals(response.exchanges, "totalDQT", function(exchange) {
                    return NRS.formatAmount(NRS.calculateOrderTotalDQT(exchange.units, exchange.rateDQT));
                });
                ...
                for (var i = 0; i < response.exchanges.length; i++) {
                    ...
                    "<td class='numeric'>" + NRS.formatQuantity(exchange.units, exchange.decimals, false, quantityDecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatOrderPricePerWholeQNT(exchange.rateDQT, exchange.decimals, rateDQTDecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatAmount(NRS.calculateOrderTotalDQT(exchange.units, exchange.rateDQT, exchange.decimals),false,false,totalDQTDecimals) + "</td>" +
                    "</tr>";
                }
                NRS.dataLoaded(rows);
            } else {
    ...
    NRS.forms.issueCurrency = function ($modal) {
        ...
        if (data.minReservePerUnitDQT) {
            data.minReservePerUnitDQT = NRS.convertToDQT(data.minReservePerUnitDQT);
            data.minReservePerUnitDQT = NRS.convertToQNTf(data.minReservePerUnitDQT, data.decimals);
        }
    ...
    NRS.forms.publishExchangeOffer = function ($modal) {
        ...
        data.buyRateDQT = NRS.calculatePricePerWholeQNT(NRS.convertToDQT(data.buyRateDQT), decimals);
        ...
        data.sellRateDQT = NRS.calculatePricePerWholeQNT(NRS.convertToDQT(data.sellRateDQT), decimals);
        return {
            "data": data
        };
    };
    ...
    $("#reserve_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency+", {
            ...
            if (response && !response.errorDescription) {
                ...
                var minReserve = response.minReservePerUnitDQT;
                var currentReserve = response.currentReservePerUnitDQT;
                ...
                var minReservePerUnitDQT = new BigInteger(minReserve).multiply(new BigInteger("" + Math.pow(10, decimals)));
                $("#reserve_currency_minReserve_text").html(NRS.formatQuantity(NRS.convertToDBN(minReservePerUnitDQT.multiply(new BigInteger(resSupply))), decimals));
                $("#reserve_currency_currentReserve").val(currentReserve);
                var currentReservePerUnitDQT = new BigInteger(currentReserve).multiply(new BigInteger("" + Math.pow(10, decimals)));
                $("#reserve_currency_currentReserve_text").html(NRS.formatQuantity(NRS.convertToDBN(currentReservePerUnitDQT.multiply(new BigInteger(resSupply))), decimals));
    ...
    reserveCurrencyAmount.blur(function () {
        ...
        var amountDQT = NRS.convertToDQT(this.value);
        var unitAmountDQT = new BigInteger(amountDQT).divide(new BigInteger(resSupply));
        var roundUnitAmountDQT = NRS.convertToDQT(NRS.amountToPrecision(NRS.convertToDBN(unitAmountDQT), decimals));
        $("#reserve_currency_total").val(NRS.convertToDBN(roundUnitAmountDQT));
        reserveCurrencyAmount.val(NRS.convertToDBN(new BigInteger(roundUnitAmountDQT).multiply(new BigInteger(resSupply)).toString()));
    });

    NRS.forms.currencyReserveIncrease = function ($modal) {
        var data = NRS.getFormData($modal.find("form:first"));
        var decimals = parseInt(data.decimals, 10);
        data.amountPerUnitDQT = NRS.calculatePricePerWholeQNT(NRS.convertToDQT(data.amountPerUnitDQT), decimals);
    ...
    $("#claim_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency", {
            ...
            var currentReservePerUnitDQT = new BigInteger(response.currentReservePerUnitDQT).multiply(new BigInteger("" + Math.pow(10, response.decimals)));
            $("#claimRate").html(NRS.formatAmount(currentReservePerUnitDQT) + " [DBN/" + currencyCode + "]");
        });
