...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		...
		return NRS.convertToDBN(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};
    ...
    NRS.calculateOrderTotalDQT = function (quantityQNT, priceDQT) {
        return calculateOrderTotalImpl(quantityQNT, priceDQT).toString();
    };

    NRS.calculateOrderTotal = function (quantityQNT, priceDQT) {
        return NRS.convertToDBN(calculateOrderTotalImpl(quantityQNT, priceDQT));
    };
    ...
    NRS.convertToDBN = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
    ...
    NRS.convertToDQT = function (currency) {
    ...
    NRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = NRS.convertToDBN(amount, true);
        ...
        if (NRS.settings) {
            ...
            var maxLength = parseInt(NRS.settings.max_dbn_decimals) + offset;
    ...
    NRS.getTransactionsAmountDecimals = function(transactions) {
        var decimals = {};
   		decimals.amount = NRS.getNumberOfDecimals(transactions, "amountDQT", function (transaction) {
   			return NRS.formatAmount(transaction.amountDQT);
   		});
   		decimals.fee = NRS.getNumberOfDecimals(transactions, "feeDQT", function (transaction) {
   			return NRS.formatAmount(transaction.feeDQT);
   		});
        return decimals;
   	};
    ...
    NRS.createInfoTable = function (data, fixed) {
		...
		for (var key in data) {
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                value = NRS.formatAmount(new BigInteger(String(value))) + " DBN";
