...
var NRS = (function (NRS, $, undefined) {
            ...
            if (transaction.type == 0) {
                switch (transaction.subtype) {
                    case 0:
                        data = {
                            ...
                            "amount": transaction.amountDQT,
                            "fee": transaction.feeDQT,
                ...
                switch (transaction.subtype) {
                    ...
                    case 6:
                        ...
                        if (transaction.attachment.priceDQT == "0") {
                        ...
                        if (type == $.t("alias_sale")) {
                            data["price"] = transaction.attachment.priceDQT
                        }
                        ...
                        if (type == $.t("alias_sale")) {
                            ...
                            NRS.sendRequest("getAlias", {
                                ...
                                if (!response.errorCode) {
                                    if (transaction.recipient != response.buyer || transaction.attachment.priceDQT != response.priceDQT) {
                                        message = $.t("alias_sale_info_outdated");
                                        messageStyle = "danger";
                                    } else if (transaction.recipient == NRS.account) {
                                        message = $.t("alias_sale_direct_offer", {
                                            "dbn": NRS.formatAmount(transaction.attachment.priceDQT)
                            ...
                            if (transaction.attachment.asset) {
                                NRS.sendRequest("getAsset", {
                                    ...
                                    data = {
                                        ...
                                        "asset_name": asset.name,
                                        "quantity": [transaction.attachment.quantityQNT, asset.decimals],
                                        "price_formatted_html": NRS.formatOrderPricePerWholeQNT(transaction.attachment.priceDQT, asset.decimals) + " DBN",
                                        "total_formatted_html": NRS.formatAmount(NRS.calculateOrderTotalDQT(transaction.attachment.quantityQNT, transaction.attachment.priceDQT)) + " DBN"
                                    };
                                    data["sender"] = transaction.senderRS ? transaction.senderRS : transaction.sender;
                                    infoTable.find("tbody").append(NRS.createInfoTable(data));
                                    infoTable.show();
                            ...
                            if (transaction.attachment.asset) {
                                NRS.sendRequest("getAsset", {
                                    ...
                                    data = {
                                        ...
                                        "asset_formatted_html": NRS.getTransactionLink(transaction.attachment.asset),
                                        "asset_name": asset.name,
                                        "amount_per_share": NRS.formatOrderPricePerWholeQNT(transaction.attachment.amountDQTPerQNT, asset.decimals) + " DBN",
                ...
                switch (transaction.subtype) {
                    case 0:
                        data = {
                            ...
                            "name": transaction.attachment.name,
                            "description": transaction.attachment.description,
                            "price": transaction.attachment.priceDQT,
                    ...
                    case 2:
                        ...
                        NRS.sendRequest("getDGSGood", {
                            ...
                            data = {
                                ...
                                "item_name": goods.name,
                                "new_price_formatted_html": NRS.formatAmount(transaction.attachment.priceDQT) + " DBN",
                                "seller": NRS.getAccountFormatted(goods, "seller")
                            };
                        ...
                        NRS.sendRequest("getDGSPurchase", {
                            ...
                            NRS.sendRequest("getDGSGood", {
                                ...
                                data = {
                                    ...
                                    "item_name": goods.name,
                                    "price": purchase.priceDQT
                                };

                                data["quantity_formatted_html"] = NRS.format(purchase.quantity);

                                if (purchase.quantity != "1") {
                                    var orderTotal = NRS.formatAmount(new BigInteger(String(purchase.quantity)).multiply(new BigInteger(String(purchase.priceDQT))));
                                    data["total_formatted_html"] = orderTotal + " DBN";
                                }

                                if (transaction.attachment.discountDQT) {
                                    data["discount"] = transaction.attachment.discountDQT;
                                }
                                ...
                                } else if (NRS.account == purchase.seller && purchase.refundDQT) {
                                    callout = $.t("purchase_refunded");
                                }
                    ...
                    case 7:
                        ...
                        NRS.sendRequest("getDGSPurchase", {
                            ...
                            NRS.sendRequest("getDGSGood", {
                                ...
                                var orderTotal = new BigInteger(String(purchase.quantity)).multiply(new BigInteger(String(purchase.priceDQT)));
                                data["order_total_formatted_html"] = NRS.formatAmount(orderTotal) + " DBN";
                                data["refund"] = transaction.attachment.refundDQT;
                                data["buyer"] = NRS.getAccountFormatted(purchase, "buyer");
                                data["seller"] = NRS.getAccountFormatted(purchase, "seller");
                                infoTable.find("tbody").append(NRS.createInfoTable(data));
                                infoTable.show();
                ...
                switch (transaction.subtype) {
                    case 0:
                        var minReservePerUnitDQT = new BigInteger(transaction.attachment.minReservePerUnitDQT).multiply(new BigInteger("" + Math.pow(10, transaction.attachment.decimals)));
                        data = {
                            ...
                            "min_reserve_per_unit_formatted_html": NRS.formatAmount(minReservePerUnitDQT) + " DBN",
                        ...
                        if (currency) {
                            ...
                            var currentReservePerUnitDQT = new BigInteger(currency.currentReservePerUnitDQT).multiply(new BigInteger("" + Math.pow(10, currency.decimals)));
                            data["current_reserve_per_unit_formatted_html"] = NRS.formatAmount(currentReservePerUnitDQT) + " DBN";
                        } else {
                    ...
                    case 1:
                        if (currency) {
                            var amountPerUnitDQT = new BigInteger(transaction.attachment.amountPerUnitDQT).multiply(new BigInteger("" + Math.pow(10, currency.decimals)));
                            var resSupply = currency.reserveSupply;
                            data = {
                                "type": $.t("reserve_increase"),
                                "code": currency.code,
                                "reserve_units": [resSupply, currency.decimals],
                                "amount_per_unit_formatted_html": NRS.formatAmount(amountPerUnitDQT) + " DBN",
                                "reserved_amount_formatted_html": NRS.formatAmount(NRS.calculateOrderTotalDQT(amountPerUnitDQT, NRS.convertToQNTf(resSupply, currency.decimals))) + " DBN"
                            };
                        } else {
                    ...
                    case 2:
                        if (currency) {
                            var reservePerUnitDQT = new BigInteger(currency.currentReservePerUnitDQT).multiply(new BigInteger("" + Math.pow(10, currency.decimals)));
                            data = {
                                "type": $.t("reserve_claim"),
                                "code": currency.code,
                                "units": [transaction.attachment.units, currency.decimals],
                                "claimed_amount_formatted_html": NRS.formatAmount(NRS.convertToQNTf(NRS.calculateOrderTotalDQT(reservePerUnitDQT, transaction.attachment.units), currency.decimals)) + " DBN"
                            };
                        } else {
                            data = NRS.getUnknownCurrencyData(transaction);
                        }
                ...
                if (!incorrect) {
                    ...
                    if (currency != null && NRS.isReservable(currency.type)) {
                        infoCallout.append("<a href='#' data-toggle='modal' data-target='#currency_founders_modal' data-currency='" + NRS.escapeRespStr(currency.currency) + "' data-name='" + NRS.escapeRespStr(currency.name) + "' data-code='" + NRS.escapeRespStr(currency.code) + "' data-ressupply='" + NRS.escapeRespStr(currency.reserveSupply) + "' data-initialsupply='" + NRS.escapeRespStr(currency.initialSupply) + "' data-decimals='" + NRS.escapeRespStr(currency.decimals) + "' data-minreserve='" + NRS.escapeRespStr(currency.minReservePerUnitDQT) + "' data-issueheight='" + NRS.escapeRespStr(currency.issuanceHeight) + "'>View Founders</a><br/>");
                    }
                    if (currency != null) {
        ...
        NRS.sendRequest("getOrderTrades", params, function (response) {
            ...
            if (response.trades && response.trades.length > 0) {
                ...
                for (var i = 0; i < response.trades.length; i++) {
                    ...
                    tradeTotal = tradeTotal.add(new BigInteger(trade.quantityQNT).multiply(new BigInteger(trade.priceDQT)));
                    rows += "<tr>" +
                    ...
                    "<td>" + NRS.calculateOrderPricePerWholeQNT(trade.priceDQT, asset.decimals) + "</td>" +
                    "<td>" + NRS.formatAmount(NRS.calculateOrderTotalDQT(trade.quantityQNT, trade.priceDQT)) +
                    "</td>" +
                    "</tr>";
                }
                rows += "</tbody></table>";
            ...
            data["total_traded"] = NRS.formatAmount(tradeTotal, false, true) + " DBN";
        }, { isAsync: false });
    ...
    NRS.formatCurrencyExchange = function (currency, transaction, type) {
        var rateUnitsStr = " [ " + currency.code + " / DBN ]";
        var data = {
            "type": type == "sell" ? $.t("sell_currency") : $.t("buy_currency"),
            "code": currency.code,
            "units": [transaction.attachment.units, currency.decimals],
            "rate": NRS.calculateOrderPricePerWholeQNT(transaction.attachment.rateDQT, currency.decimals) + rateUnitsStr
        };
        var rows = "";
        NRS.sendRequest("getExchangesByExchangeRequest", {
            ...
            if (response.exchanges && response.exchanges.length > 0) {
                ...
                for (var i = 0; i < response.exchanges.length; i++) {
                    ...
                    exchangedTotal = exchangedTotal.add(new BigInteger(exchange.units).multiply(new BigInteger(exchange.rateDQT)));
                    rows += "<tr>" +
                    ...
                    "<td>" + NRS.calculateOrderPricePerWholeQNT(exchange.rateDQT, currency.decimals) + "</td>" +
                    "<td>" + NRS.formatAmount(NRS.calculateOrderTotalDQT(exchange.units, exchange.rateDQT)) +
                    "</td>" +
                    "</tr>";
                }
                rows += "</tbody></table>";
            ...
            data["total_exchanged"] = NRS.formatAmount(exchangedTotal, false, true) + " [DBN]";
        }, { isAsync: false });
        return data;
    };
    ...
    NRS.formatCurrencyOffer = function (currency, transaction) {
        ...
        if (buyOffer && sellOffer) {
            data = {
                ...
                "buy_rate_formatted_html": NRS.calculateOrderPricePerWholeQNT(transaction.attachment.buyRateDQT, currency.decimals) + rateUnitsStr,
                ...
                "sell_rate_formatted_html": NRS.calculateOrderPricePerWholeQNT(transaction.attachment.sellRateDQT, currency.decimals) + rateUnitsStr,
