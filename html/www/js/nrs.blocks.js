...
var NRS = (function(NRS, $) {
	...
	NRS.blocksPageLoaded = function(blocks) {
		...
		for (var i = 0; i < blocks.length; i++) {
			var block = blocks[i];
			totalAmount = totalAmount.add(new BigInteger(block.totalAmountDQT));
			totalFees = totalFees.add(new BigInteger(block.totalFeeDQT));
			...
			rows += "<tr>" +
                ...
                "<td>" + NRS.formatAmount(block.totalAmountDQT) + "</td>" +
                "<td>" + NRS.formatAmount(block.totalFeeDQT) + "</td>" +
        ...
        if (NRS.blocksPageType == "forged_blocks") {
			NRS.sendRequest("getAccountBlockCount+", {
				...
				if (response.numberOfBlocks && response.numberOfBlocks > 0) {
                    ...
                    var avgFee = new Big(NRS.accountInfo.forgedBalanceDQT).div(response.numberOfBlocks).div(new Big("100000000")).toFixed(2);
                    $("#blocks_average_fee").html(NRS.formatStyledAmount(NRS.convertToDQT(avgFee))).removeClass("loading_dots");
				} else {
					$("#forged_blocks_total").html(0).removeClass("loading_dots");
					$("#blocks_average_fee").html(0).removeClass("loading_dots");
				}
			});
			$("#forged_fees_total").html(NRS.formatStyledAmount(NRS.accountInfo.forgedBalanceDQT)).removeClass("loading_dots");
			...
			averageFee = NRS.convertToDQT(averageFee);
			averageAmount = NRS.convertToDQT(averageAmount);
			if (time == 0) {
