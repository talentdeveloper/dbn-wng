...
var NRS = (function(NRS, $) {
			...
			html += "<table width='100%' style='table-layout:fixed;'>";
				html += "<tr>";
					...
					html += "<td>";
						...
						html += "<div class='price'><strong>" + NRS.formatAmount(good.priceDQT) + " DBN</strong></div>";
	...
	NRS.getMarketplacePurchaseHTML = function(purchase, showBuyer) {
		...
		return "<div data-purchase='" + NRS.escapeRespStr(purchase.purchase) + "' " + (purchase.unconfirmed ? "class='tentative'" : "") + "><div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>" +
			...
			"<tr><td><strong>" + $.t("price") + "</strong>:</td><td>" + NRS.formatAmount(purchase.priceDQT) + " DBN</td></tr>" +
	...
	NRS.dgs_search_main = function(callback) {
		...
		$(".dgs_search input[name=q]").val("").trigger("unmask").mask("DBN-****-****-****-*****");
		...
		NRS.sendRequest("getDGSGoods+", {
         ...
         var rows = "";
			if (response.goods && response.goods.length) {
				...
				var priceDecimals = NRS.getNumberOfDecimals(response.goods, "priceDQT", function(val) {
					return NRS.formatAmount(val.priceDQT);
				});
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<td class='price numeric'>" + NRS.formatAmount(good.priceDQT, false, false, priceDecimals) + " DBN</td>";
	...
	NRS.forms.dgsPriceChangeComplete = function(response, data) {
		...
		$("#my_dgs_listings_table").find("tr[data-goods=" + String(data.goods).escapeHTML() + "]").addClass("tentative").find(".price").html(NRS.formatAmount(data.priceDQT) + " DBN");
	};
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGood", {
					...
					if (good.errorCode) {
						...
						output += "<tr><th><strong>" + $.t("price") + "</strong>:</th><td>" + NRS.formatAmount(response.priceDQT) + " DBN</td></tr>";
						...
						if (type == "dgs_refund_modal" || type == "dgs_delivery_modal" || type == "dgs_feedback_modal") {
							...
							if (response.quantity != "1") {
								var orderTotal = NRS.formatAmount(new BigInteger(String(response.quantity)).multiply(new BigInteger(String(response.priceDQT))));
								output += "<tr><th><strong>" + $.t("total") + "</strong>:</th><td>" + orderTotal + " DBN</td></tr>";
							}
							if (response.discountDQT && (type == "dgs_refund_modal" || type == "dgs_feedback_modal")) {
								output += "<tr><th><strong>" + $.t("discount") + "</strong>:</th><td>" + NRS.formatAmount(response.discountDQT) + " DBN</td></tr>";
							}
						}
						...
						if (type == "dgs_view_refund_modal") {
							output += "<tr><th><strong>" + $.t("refund_price") + "</strong>:</th><td>" + NRS.formatAmount(response.refundDQT) + " DBN</td></tr>";
						}
						...
						if (type == "dgs_refund_modal") {
							var orderTotalBeforeDiscount = new BigInteger(String(response.quantity)).multiply(new BigInteger(String(response.priceDQT)));
							var refund = orderTotalBeforeDiscount.subtract(new BigInteger(String(response.discountDQT)));

							$("#dgs_refund_purchase").val(response.purchase);
							$("#dgs_refund_refund").val(NRS.convertToDBN(refund));
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGoodsPurchases+", {
					...
					if (type == "dgs_quantity_change_modal") {
						...
						$("#dgs_price_change_current_price, #dgs_price_change_price").val(NRS.convertToDBN(response.priceDQT).escapeHTML());
					} else if (type == "dgs_purchase_modal") {
						$modal.find("input[name=recipient]").val(response.sellerRS);

						$("#dgs_purchase_price").val(NRS.escapeRespStr(response.priceDQT));
						$("#dgs_total_purchase_price").html(NRS.formatAmount(response.priceDQT) + " DBN");

						$("#dgs_purchase_quantity").on("change", function() {
							var totalDQT = new BigInteger(response.priceDQT).multiply(new BigInteger(String($(this).val()))).toString();
							$("#dgs_total_purchase_price").html(NRS.formatAmount(totalDQT) + " DBN");
						});
					}
				});
			}
	...
	$(".dgs_search").on("submit", function(e) {
		...
		} else if (/^(DBN\-)/i.test(seller)) {
			var address = new DbnAddress();
	...
	NRS.dgs_listings = function (table, api, full) {
    		...
    		NRS.sendRequest(api, params, function (response) {
				...
				var priceDecimals = NRS.getNumberOfDecimals(response, "priceDQT", function(val) {
					return NRS.formatAmount(val.priceDQT);
				});
    			for (var i = 0; i < response.length; i++) {
    				...
    				view.data.push({
    					...
    					"good": good,
    					"price": NRS.formatAmount(item.priceDQT, NRS.decimals, false, priceDecimals),
    					"account": account,
    					"image": image
    				})
    			}
