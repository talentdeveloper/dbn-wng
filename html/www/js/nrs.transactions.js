...
var NRS = (function(NRS, $, undefined) {
				...
				if (responsePoll.transaction) {
					NRS.sendRequest("getPhasingPollVote", {
						...
						if (vm == 1) {
							$popoverTypeTR.find("td:first").html($.t('accounts', 'Accounts') + ":");
							$popoverTypeTR.find("td:last").html(String(attachment.phasingWhitelist ? attachment.phasingWhitelist.length : ""));
							votesFormatted = NRS.convertToDBN(responsePoll.result) + " / " + NRS.convertToDBN(attachment.phasingQuorum) + " DBN";
							$popoverVotesTR.find("td:last").html(votesFormatted);
						}
						if (mbModel == 1) {
							if (minBalance > 0) {
								minBalanceFormatted = NRS.convertToDBN(minBalance) + " DBN";
    ...
    NRS.getTransactionRowHTML = function(t, actions, decimals) {
		...
		if (t.type == 1 && t.subtype == 6 && t.attachment.priceDQT == "0") {
		...
		var amount = "";
		var sign = 0;
		var fee = new BigInteger(t.feeDQT);
		var feeColor = "";
		var receiving = t.recipient == NRS.account && !(t.sender == NRS.account);
		if (receiving) {
			if (t.amountDQT != "0") {
				amount = new BigInteger(t.amountDQT);
				sign = 1;
			}
			...
			if (t.sender != t.recipient) {
				...
				if (t.amountDQT != "0") {
					amount = new BigInteger(t.amountDQT); // send to myself
				}
			}
			feeColor = "color:red;";
		}
