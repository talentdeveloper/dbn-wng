...
var NRS = (function(NRS, $, undefined) {
    ...
    NRS.forms.dividendPayment = function($modal) {
        ...
        if (!data.amountDBNPerShare) {
            return {
                "error": $.t("error_amount_per_share_required")
            }
        } else {
            data.amountDQTPerQNT = NRS.calculatePricePerWholeQNT(
                NRS.convertToDQT(data.amountDBNPerShare),
    ...
    $("#dividend_payment_amount_per_share, #dividend_payment_height").on("blur", function() {
        ...
        var amountDBNPerShare = $modal.find("#dividend_payment_amount_per_share").val();
        ...
        if (amountDBNPerShare && /^\d+$/.test(height)) {
            NRS.getAssetAccounts(NRS.getCurrentAsset().asset, height,
                function (response) {
                    ...
                    var priceDQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToDQT(amountDBNPerShare), NRS.getCurrentAsset().decimals));
                    var totalDBN = NRS.calculateOrderTotal(totalQuantityQNT, priceDQT);
