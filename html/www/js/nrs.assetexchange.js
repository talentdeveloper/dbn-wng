...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^DBN\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^DBN\-/i.test(data.id)) {
    ...
    NRS.loadAsset = function (asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_dbn").html($.t("sell_asset_for_dbn", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
            $("#buy_asset_with_dbn").html($.t("buy_asset_with_dbn", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
        ...
        if (NRS.accountInfo.unconfirmedBalanceDQT == "0") {
            $("#your_dbn_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_dbn_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceDQT));
    ...
    function processOrders(orders, type, refresh) {
        if (orders.length) {
            ...
            var priceDecimals = NRS.getNumberOfDecimals(orders, "priceDQT", function(val) {
                return NRS.formatOrderPricePerWholeQNT(val.priceDQT, currentAsset.decimals);
            });
            var amountDecimals = NRS.getNumberOfDecimals(orders, "totalDQT", function(val) {
                return NRS.formatAmount(NRS.calculateOrderTotalDQT(val.quantityQNT, val.priceDQT));
            });
            for (var i = 0; i < orders.length; i++) {
                order = orders[i];
                order.priceDQT = new BigInteger(order.priceDQT);
                order.quantityQNT = new BigInteger(order.quantityQNT);
                order.totalDQT = new BigInteger(NRS.calculateOrderTotalDQT(order.quantityQNT, order.priceDQT));
                sum = sum.add(order.totalDQT);
                if (i == 0 && !refresh) {
                    $("#" + (type == "ask" ? "buy" : "sell") + "_asset_price").val(NRS.calculateOrderPricePerWholeQNT(order.priceDQT, currentAsset.decimals));
                }
                var statusIcon = NRS.getTransactionStatusIcon(order);
                var className = (order.account == NRS.account ? "your-order" : "");
                rows += "<tr class='" + className + "' data-transaction='" + NRS.escapeRespStr(order.order) + "' data-quantity='" + order.quantityQNT.toString().escapeHTML() + "' data-price='" + order.priceDQT.toString().escapeHTML() + "'>" +
                    ...
                    "<td class='numeric'>" + NRS.formatOrderPricePerWholeQNT(order.priceDQT, currentAsset.decimals, priceDecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatAmount(order.totalDQT, false, false, amountDecimals) + "</td>" +
    ...
    NRS.loadAssetOrders = function (type, assetId, refresh) {
        ...
        function(err, results) {
            ...
            orders.sort(function (a, b) {
                if (type == "ask") {
                    return a.priceDQT - b.priceDQT;
                } else {
                    return b.priceDQT - a.priceDQT;
                }
            });
    ...
    NRS.getAssetDividendHistory = function (assetId, table) {
        ...
        NRS.sendRequest("getAssetDividends+", options, function (response) {
            ...
            var amountDQTPerQNTDecimals = NRS.getNumberOfDecimals(dividends, "amountDQTPerQNT", function(val) {
                return NRS.formatOrderPricePerWholeQNT(val.amountDQTPerQNT, currentAsset.decimals);
            });
            for (var i = 0; i < dividends.length; i++) {
                var dividend = dividends[i];
                dividend.numberOfAccounts = new BigInteger(dividend.numberOfAccounts.toString());
                dividend.amountDQTPerQNT = new BigInteger(dividend.amountDQTPerQNT);
                ...
                view.data.push({
                    ...
                    "amount_per_share": NRS.formatOrderPricePerWholeQNT(dividend.amountDQTPerQNT, currentAsset.decimals, amountDQTPerQNTDecimals)
                })
            }
            view.render({
                isLoading: false,
    ...
    NRS.getAssetTradeHistory = function (assetId, refresh) {
        ...
        NRS.sendRequest("getTrades+" + assetId, options, function (response) {
            ...
            if (response.trades && response.trades.length) {
                ...
                var priceDecimals = NRS.getNumberOfDecimals(trades, "priceDQT", function(val) {
                    return NRS.formatOrderPricePerWholeQNT(val.priceDQT, currentAsset.decimals);
                });
                ...
                for (var i = 0; i < trades.length; i++) {
                    var trade = trades[i];
                    trade.priceDQT = new BigInteger(trade.priceDQT);
                    trade.quantityQNT = new BigInteger(trade.quantityQNT);
                    trade.totalDQT = new BigInteger(NRS.calculateOrderTotalDQT(trade.priceDQT, trade.quantityQNT));
                    rows += "<tr>" +
                        ...
                        "<td class='asset_price numeric'>" + NRS.formatOrderPricePerWholeQNT(trade.priceDQT, currentAsset.decimals, priceDecimals) + "</td>" +
                    ...
                    rows += "'>" + NRS.formatAmount(trade.totalDQT, false, false, amountDecimals) + "</td>" +
    ...
    $("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function (e) {
        ...
        try {
            var priceDQT = new BigInteger(String($tr.data("price")));
            var quantityQNT = new BigInteger(String($tr.data("quantity")));
            var totalDQT = new BigInteger(NRS.calculateOrderTotalDQT(quantityQNT, priceDQT));

            $("#" + type + "_asset_price").val(NRS.calculateOrderPricePerWholeQNT(priceDQT, currentAsset.decimals));
            $("#" + type + "_asset_quantity").val(NRS.convertToQNTf(quantityQNT, currentAsset.decimals));
            $("#" + type + "_asset_total").val(NRS.convertToDBN(totalDQT));
        } catch (err) {
            return;
        }
        ...
        if (type == "buy") {
            try {
                var balanceDQT = new BigInteger(NRS.accountInfo.unconfirmedBalanceDQT);
            } catch (err) {
                return;
            }

            if (totalDQT.compareTo(balanceDQT) > 0) {
    ...
    $("#sell_automatic_price, #buy_automatic_price").on("click", function () {
        try {
            ...
            var price = new Big(NRS.convertToDQT(String(assetPrice.val())));
            var balanceDQT = new Big(NRS.accountInfo.unconfirmedBalanceDQT);
            var maxQuantity = new Big(NRS.convertToQNTf(currentAsset.quantityQNT, currentAsset.decimals));
            if (balanceDQT.cmp(new Big("0")) <= 0) {
                return;
            }
            ...
            if (price.cmp(new Big("0")) <= 0) {
                ...
                assetPrice.val(NRS.convertToDBN(price.toString()));
            }
            ...
            if (type == "sell") {
                ...
                quantity = new Big(NRS.amountToPrecision(balanceDQT.div(price).toString(), currentAsset.decimals));
            }
            ...
            assetTotal.val(NRS.convertToDBN(total.toString()));
            assetTotal.css({
                "background": "",
                "color": ""
            });
    ...
    $("#sell_asset_quantity, #sell_asset_price, #buy_asset_quantity, #buy_asset_price").keyup(function () {
        ...
        try {
            ...
            var priceDQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToDQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));

            if (priceDQT.toString() == "0" || quantityQNT.toString() == "0") {
                $("#" + orderType + "_asset_total").val("0");
            } else {
                var total = NRS.calculateOrderTotal(quantityQNT, priceDQT, currentAsset.decimals);
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var priceDQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToDQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));
            var totalDBN = NRS.formatAmount(NRS.calculateOrderTotalDQT(quantityQNT, priceDQT, currentAsset.decimals), false, true);
        } catch (err) {
        ...
        var priceDQTPerWholeQNT = priceDQT.multiply(new BigInteger("" + Math.pow(10, currentAsset.decimals)));
        var description;
        var tooltipTitle;
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "dbn": NRS.formatAmount(priceDQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "dbn": NRS.formatAmount(priceDQTPerWholeQNT, false, true),
                "total_dbn": totalDBN
            });
        } else {
        ...
        $("#asset_order_total").html(totalDBN + " DBN");
        ...
        $("#asset_order_price").val(priceDQT.toString());
    });
    ...
    NRS.pages.trade_history = function () {
        NRS.sendRequest("getTrades+", {
            ...
            if (response.trades && response.trades.length) {
                ...
                var priceDecimals = NRS.getNumberOfDecimals(trades, "priceDQT", function(val) {
                    return NRS.formatOrderPricePerWholeQNT(val.priceDQT, val.decimals);
                });
                var amountDecimals = NRS.getNumberOfDecimals(trades, "totalDQT", function(val) {
                    return NRS.formatAmount(NRS.calculateOrderTotalDQT(val.quantityQNT, val.priceDQT));
                });
                var rows = "";
                for (var i = 0; i < trades.length; i++) {
                    ...
                    rows += "<tr>" +
                        ...
                        "<td class='asset_price numeric'>" + NRS.formatOrderPricePerWholeQNT(trade.priceDQT, trade.decimals, priceDecimals) + "</td>" +
                        "<td style='" + (type == "buy" ? "color:red" : "color:green") + "' class='numeric'>" + NRS.formatAmount(trade.totalDQT, false, false, amountDecimals) + "</td>" +
            ...
            for (var i = 0; i < NRS.accountInfo.assetBalances.length; i++) {
                ...
                NRS.sendRequest("getAskOrders+", {
                    ...
                    if (response.askOrders && response.askOrders.length) {
                        result.ask_orders[input.asset] = new BigInteger(response.askOrders[0].priceDQT);
                    } else {
                ...
                NRS.sendRequest("getBidOrders+", {
                    ...
                    if (response.bidOrders && response.bidOrders.length) {
                        result.bid_orders[input.asset] = new BigInteger(response.bidOrders[0].priceDQT);
                    } else {
    ...
    NRS.myAssetsPageLoaded = function (result) {
        ...
        var valueDecimals = NRS.getNumberOfDecimals(result.assets, "bid", function(asset) {
            ...
            var totalDQT = new BigInteger(NRS.calculateOrderTotalDQT(asset.balanceQNT, asset.bid_orders));
            return NRS.formatAmount(totalDQT);
        });
        for (var i = 0; i < result.assets.length; i++) {
            ...
            if (highestBidOrder != -1) {
                var totalDQT = new BigInteger(NRS.calculateOrderTotalDQT(asset.balanceQNT, highestBidOrder));
            }
            rows += "<tr data-asset='" + NRS.escapeRespStr(asset.asset) + "'>" +
                ...
                "<td class='numeric'>" + (highestBidOrder != -1 ? NRS.formatAmount(totalDQT, false, false, valueDecimals) : "") + "</td>" +
                "<td>" +
    ...
    NRS.openOrdersLoaded = function (orders, type, callback) {
        ...
        for (var i = 0; i < orders.length; i++) {
            ...
            completeOrder.priceDQT = new BigInteger(completeOrder.priceDQT);
            completeOrder.quantityQNT = new BigInteger(completeOrder.quantityQNT);
            completeOrder.totalDQT = new BigInteger(NRS.calculateOrderTotalDQT(completeOrder.quantityQNT, completeOrder.priceDQT));
            rows += "<tr data-order='" + NRS.escapeRespStr(completeOrder.order) + "'><td><a href='#' data-goto-asset='" + NRS.escapeRespStr(completeOrder.asset) + "'>" + NRS.escapeRespStr(completeOrder.assetName) + "</a></td><td>" + NRS.formatQuantity(completeOrder.quantityQNT, completeOrder.decimals) + "</td><td>" + NRS.formatOrderPricePerWholeQNT(completeOrder.priceDQT, completeOrder.decimals) + "</td><td>" + NRS.formatAmount(completeOrder.totalDQT) + "</td><td class='cancel'><a href='#' data-toggle='modal' data-target='#cancel_order_modal' data-order='" + NRS.escapeRespStr(completeOrder.order) + "' data-type='" + type + "'>" + $.t("cancel") + "</a></td></tr>";
        }
