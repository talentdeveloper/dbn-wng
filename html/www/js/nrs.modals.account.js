...
var NRS = (function(NRS, $) {
	...
	NRS.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceDQT == "0") {
			$("#user_info_modal_account_balance").html("0");
		} else {
			$("#user_info_modal_account_balance").html(NRS.formatAmount(account.unconfirmedBalanceDQT) + " DBN");
		}
    ...
    NRS.userInfoModal.transactions = function() {
        NRS.sendRequest("getBlockchainTransactions", {
			...
			if (response.transactions && response.transactions.length) {
				var rows = "";
				var amountDecimals = NRS.getNumberOfDecimals(response.transactions, "amountDQT", function(val) {
					return NRS.formatAmount(val.amountDQT);
				});
				...
				for (var i = 0; i < response.transactions.length; i++) {
                    ...
                    var receiving;
					if (/^DBN\-/i.test(String(NRS.userInfoModal.user))) {
						receiving = (transaction.recipientRS == NRS.userInfoModal.user);
					} else {
						receiving = (transaction.recipient == NRS.userInfoModal.user);
					}

					if (transaction.amountDQT) {
						transaction.amount = new BigInteger(transaction.amountDQT);
						transaction.fee = new BigInteger(transaction.feeDQT);
					}
		...
		NRS.sendRequest("getDGSGoods", {
			...
			if (response.goods && response.goods.length) {
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<tr><td><a href='#' data-goto-goods='" + NRS.escapeRespStr(good.goods) + "' data-seller='" + NRS.escapeRespStr(NRS.userInfoModal.user) + "'>" + NRS.escapeRespStr(good.name) + "</a></td><td class='numeric'>" + NRS.formatAmount(good.priceDQT, false, false, priceDecimals) + " DBN</td><td class='numeric'>" + NRS.format(good.quantity, false, quantityDecimals) + "</td></tr>";
				}
			}
