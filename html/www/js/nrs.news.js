	...
	NRS.pages.news = function() {
      ...
      var settingsReddit = {
         ...
         "filter": function(entry, tokens) {
            return tokens.title.indexOf("Donations toward") == -1 && tokens.title.indexOf("DBN tipping bot has arrived") == -1
         },
         "ssl": ssl
      };

      $("#dbnforum_news").rss("https://dbnforum.org/index.php?type=rss;action=.xml", settings, NRS.newsLoaded);
      $("#reddit_news").rss("http://www.reddit.com/r/DBN/.rss", settingsReddit, NRS.newsLoaded);
      $("#dbncoin_blogspot_news").rss("http://dbncoin.blogspot.com/feeds/posts/default", settings, NRS.newsLoaded);
      $("#dbner_news").rss("http://dbner.org/feed/", settings, NRS.newsLoaded);
		NRS.pageLoaded();
	};
