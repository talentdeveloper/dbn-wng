...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedDbnAccounts") && NRS.getStrItem("savedDbnAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedDbnAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (type == "account") {
            ...
            NRS.scanQRCode(readerId, function(text) {
                var dbnAddress = new DbnAddress();
                if (dbnAddress.set(text)) {
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedDbnAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedDbnAccounts');
		} else {
			NRS.setStrItem("savedDbnAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedDbnAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedDbnAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedDbnAccounts", accountsStr + account + ";");
    }
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if (NRS.getStrItem("savedDbnAccounts") && NRS.getStrItem("savedDbnAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedDbnAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Dbn passphrase.";
			html += "</p>";
			...
			html += "you can loose your DBN! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of DBN now or in the future.";
			html += "</p>";
			html += "</div>";
