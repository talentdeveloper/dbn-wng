...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumDBN",
			addonText: "DBN",
			addonI18n: "dbn_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_dbn', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceDBN';
		context['addonText'] = 'DBN';
		context['addonI18n'] = 'dbn_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_dbn', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
