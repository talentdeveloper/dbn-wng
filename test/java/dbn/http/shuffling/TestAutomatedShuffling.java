...
import dbn.Dbn;
import dbn.Shuffler;
import dbn.Shuffling;
import dbn.ShufflingTransaction;
import dbn.Tester;
import dbn.Transaction;
import dbn.crypto.AnonymouslyEncryptedData;
...
import static dbn.http.shuffling.ShufflingUtil.ALICE_RECIPIENT;
import static dbn.http.shuffling.ShufflingUtil.BOB_RECIPIENT;
import static dbn.http.shuffling.ShufflingUtil.CHUCK_RECIPIENT;
import static dbn.http.shuffling.ShufflingUtil.DAVE_RECIPIENT;
import static dbn.http.shuffling.ShufflingUtil.broadcast;
import static dbn.http.shuffling.ShufflingUtil.cancel;
import static dbn.http.shuffling.ShufflingUtil.create;
import static dbn.http.shuffling.ShufflingUtil.createAssetShuffling;
import static dbn.http.shuffling.ShufflingUtil.createCurrencyShuffling;
import static dbn.http.shuffling.ShufflingUtil.defaultHoldingShufflingAmount;
import static dbn.http.shuffling.ShufflingUtil.defaultShufflingAmount;
import static dbn.http.shuffling.ShufflingUtil.getShuffling;
import static dbn.http.shuffling.ShufflingUtil.getShufflingParticipants;
import static dbn.http.shuffling.ShufflingUtil.process;
import static dbn.http.shuffling.ShufflingUtil.register;
import static dbn.http.shuffling.ShufflingUtil.shufflingAsset;
import static dbn.http.shuffling.ShufflingUtil.shufflingCurrency;
import static dbn.http.shuffling.ShufflingUtil.startShuffler;
import static dbn.http.shuffling.ShufflingUtil.stopShuffler;
import static dbn.http.shuffling.ShufflingUtil.verify;
...
public class TestAutomatedShuffling extends BlockchainTest {
    ...
    public void successfulShuffling() {
        ...
        Assert.assertEquals(-(defaultShufflingAmount + 11 * Constants.ONE_DBN), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + Constants.ONE_DBN), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, CHUCK.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + Constants.ONE_DBN), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + Constants.ONE_DBN), DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), ALICE.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), BOB.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), DAVE.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_DBN), DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(48 * Constants.ONE_DBN, FORGY.getBalanceDiff());
        Assert.assertEquals(48 * Constants.ONE_DBN, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void successfulAssetShuffling() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), BOB.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), DAVE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN), DAVE.getUnconfirmedBalanceDiff());

        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, ALICE_RECIPIENT.getBalanceDiff());
        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, ALICE_RECIPIENT.getUnconfirmedBalanceDiff());
        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, BOB_RECIPIENT.getBalanceDiff());
        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, BOB_RECIPIENT.getUnconfirmedBalanceDiff());
        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, CHUCK_RECIPIENT.getBalanceDiff());
        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, CHUCK_RECIPIENT.getUnconfirmedBalanceDiff());
        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, DAVE_RECIPIENT.getBalanceDiff());
        Assert.assertEquals(Constants.SHUFFLING_DEPOSIT_DQT, DAVE_RECIPIENT.getUnconfirmedBalanceDiff());
    ...
    public void processingNotStarted() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + Constants.ONE_DBN), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + Constants.ONE_DBN), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, CHUCK.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(4 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals(4 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void tooManyParticipants() {
        ...
        Assert.assertEquals(3 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals(3 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void processingNotFinished() {
        ...
        Assert.assertEquals(-11 * Constants.ONE_DBN, ALICE.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, CHUCK.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + Constants.ONE_DBN), DAVE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + Constants.ONE_DBN), DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(34 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals(34 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void verifyNotFinished() {
        ...
        Assert.assertEquals(-12 * Constants.ONE_DBN, ALICE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 11 * Constants.ONE_DBN), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 11 * Constants.ONE_DBN), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(47 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals(47 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void cancelAfterVerifyChuck() {
        ...
        Assert.assertEquals(-22 * Constants.ONE_DBN, ALICE.getBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_DBN, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 21 * Constants.ONE_DBN), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 21 * Constants.ONE_DBN), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(77 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals(77 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void cancelAfterVerifyChuckInvalidKeysAlice() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 22 * Constants.ONE_DBN), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 22 * Constants.ONE_DBN), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_DBN, CHUCK.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_DBN, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
    ...
    public void badProcessDataAlice() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 11 * Constants.ONE_DBN), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 11 * Constants.ONE_DBN), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, CHUCK.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(24 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals(24 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void modifiedProcessDataBob() {
        ...
        Assert.assertEquals(-21 * Constants.ONE_DBN, ALICE.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_DBN, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + (bobCancelFailed ? 11 : 21) * Constants.ONE_DBN), BOB.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + (bobCancelFailed ? 11 : 21) * Constants.ONE_DBN), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, CHUCK.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals((bobCancelFailed ? 44 : 54) * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals((bobCancelFailed ? 44 : 54) * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void duplicateProcessDataBob() {
        ...
        Assert.assertEquals(-21 * Constants.ONE_DBN, ALICE.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_DBN, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 21 * Constants.ONE_DBN), BOB.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 21 * Constants.ONE_DBN), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, CHUCK.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(54 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals(54 * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void modifiedProcessDataChuck() {
        ...
        Assert.assertEquals(-21 * Constants.ONE_DBN, ALICE.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_DBN, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + (chuckCancelFailed ? 11 : 21) * Constants.ONE_DBN), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + (chuckCancelFailed ? 11 : 21) * Constants.ONE_DBN), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals((chuckCancelFailed ? 65 : 75) * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getBalanceDiff());
        Assert.assertEquals((chuckCancelFailed ? 65 : 75) * Constants.ONE_DBN + Constants.SHUFFLING_DEPOSIT_DQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void modifiedRecipientKeysDave() {
        ...
        Assert.assertEquals(-65 * Constants.ONE_DBN, ALICE.getBalanceDiff() + BOB.getBalanceDiff() + CHUCK.getBalanceDiff());
    ...
    public void duplicateProcessDataChuck() {
        ...
        Assert.assertEquals(-11 * Constants.ONE_DBN, BOB.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_DBN, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + Constants.ONE_DBN), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + Constants.ONE_DBN), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_DBN, DAVE.getUnconfirmedBalanceDiff());
    ...
    public void maxShufflingSize() {
        ...
        for (int i = 0; i < n + 5; i++) {
            generateBlock();
            Block block = Dbn.getBlockchain().getLastBlock();
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN) + 100 * Constants.ONE_DBN, participants[n - 1].getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_DQT + 12 * Constants.ONE_DBN) + 100 * Constants.ONE_DBN, participants[n - 1].getUnconfirmedBalanceDiff());
